...
You will need the file `lig.pdb`. This is provided in [`foundry/models/rfd3/docs/enzyme_tutorial_files/`](enzyme_tutorial_files/1euv_lig.pdb). You can clone the [`foundry`](https://github.com/RosettaCommons/foundry) repository to easily access files related to this tutorial. 
...
If you would like to compare your outputs against those generated by the authors of this tutorial, you can find pre-generated output files in `foundry/models/rfd3/docs/enzyme_tutorial_files`.
...
1. Like all designs you will create using RFD3, we need to start by giving our calculation a name. It should be short, but descriptive, so let's call it `cys_1euv_lig`. Add this name in quotes to your file and place a colon and another pair of curly brackets after this. Your file should now look like: 
    ...
    ```
1. Next we need to specify the structure file (PDB, CIFF, etc.) that contains information about any input structures related to our calculation: 
    ```json
    "input": "./1euv_lig.pdb",
    ```
...
1. To define where our protein should be centered, we will give RFD3 an 'ori token'. This specifies the *ori*gin (center of mass) of the designed portion of our output structure:
    ...
    "ori_token": [0,1,0],
    ```
    <!-- TODO: Add image of ORI token here -->
    ```{important}
...
1. RFD3 allows for RASA conditioning to control how exposed or buried different portions of your input are relative to the designed protein.
    <!-- TODO: Add image of buried vs. exposed in one of the designed proteins -->
    ```json
    "select_buried": {
...
1. Next we're also going to unfix the *sequence* for residues A579 and A581. This will allow their side chains to change. You'll notice that the atoms from these residues passed to `select_fixed_atoms` are only the backbone atoms for these residues, so there should be no issues from using both of these options. 
    <!-- TODO: ask Saman why this is useful, does the unindex at the beginning still keep 579-581 together? -->
    <!-- TODO: add image with atom labels -->
    ```json
    "select_unfixed_sequence": "A579,A581",
    ```
1. To ensure that none of the other input residues have their side chains redesigned, we will set `redesign_motif_sidechains` to False. 
    <!-- TODO: ask Saman why this is included, from what I can tell the default is false.-->
    ```json
...
```
Your output files will be placed in a new directory `enzyme_tutorial_outputs/0`. If you run the tutorial again, change the `0` to another number to not overwrite your outputs. Your output files will be named `enzyme_tutorial_cys_1euv_lig_0_model_n.cif.gz` where `n` is the number of the design. `enzyme_tutorial` comes from the name of the JSON file and `cys_1euv_lig` comes from the name you gave your calculation in the JSON file.
...
<!-- TODO: add image of the outputs-->

You'll notice:
- The fixed atoms have stayed in place, once the structures have been aligned to the ligand
- In almost al the structures the exposed and buried atom selection was honored
