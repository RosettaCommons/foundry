...
integration_baselines/
  agag_from_json/
    agag_from_json_summary_confidences.json
```
...
`rf3 fold` automatically creates an `agag_from_json/` subdirectory inside `out_dir`, so
the output lands at `integration_baselines/agag_from_json/agag_from_json_summary_confidences.json`
