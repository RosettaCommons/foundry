...
class MSARowAttentionWithBias(nn.Module):
    ...
    def forward(self, msa, pair):  # TODO: make this as tied-attention
        ...
        if self.nseq_normalization:
            key = (
                key * self.scaling * 1 / math.sqrt(N)
            )  # fd: from nate, change to match msa xformer paper
        else:
...
class MSAColAttention(nn.Module):
    ...
    def forward(self, msa):
        ...
        query = rearrange(
            self.to_q(msa), "b n l (h d_h) -> (b l) h n d_h", h=self.n_head
        )
        key = rearrange(self.to_k(msa), "b n l (h d_h) -> (b l) h n d_h", h=self.n_head)
        value = rearrange(
            self.to_v(msa), "b n l (h d_h) -> (b l) h n d_h", h=self.n_head
        )
        ...
        with torch.backends.cuda.sdp_kernel(
            enable_flash=use_flash, enable_math=True, enable_mem_efficient=False
        ):
            attn = F.scaled_dot_product_attention(
                query, key, value, scale=self.scale_factor
            )  # (BL, h, N, N) -> (BL, h, N, d_h)
...
class MSAColGlobalAttention(nn.Module):
    ...
    def __init__(self, d_msa=64, n_head=8, d_hidden=8, bias=True):
        ...
        self.to_q = nn.Linear(d_msa, n_head * d_hidden, bias=False)
        self.to_k = nn.Linear(
            d_msa, d_hidden, bias=False
        )  # Note that the key is not multi-headed
        self.to_v = nn.Linear(
            d_msa, d_hidden, bias=False
        )  # Note that the value is not multi-headed
...
class BiasedAxialAttention(nn.Module):
    def __init__(
        self, d_pair, d_bias, n_head, d_hidden, p_drop=0.1, is_row=True, bias=True
    ):
