...
from rf2aa.model.AF3_structure import PairformerBlock, linearNoBias
...
class ConfidenceHead(nn.Module):
    ...
    def __init__(
        ...
        self.predict_exp_resolved = linearNoBias(
            c_s, ChemData().NHEAVY * n_bins_exp_resolved
        )
...
def calc_Cb_distances(X_pred_L, seq, rep_atoms, frame_atom_idxs):
    ...
    is_valid_Cb = (
        (seq != ChemData().aa2num["UNK"])
        & (seq != ChemData().aa2num["GLY"])
        & (seq != ChemData().aa2num["MAS"])
    )
