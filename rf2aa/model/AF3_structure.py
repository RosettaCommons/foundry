...
class DiffusionModule(nn.Module):
    def __init__(self, sigma_data, c_atom, c_atompair, c_token, c_s, c_z, f_pred, diffusion_conditioning, atom_attention_encoder, diffusion_transformer, atom_attention_decoder):
        ...
        self.atom_attention_encoder = AtomAttentionEncoderDiffusion(c_token=c_token, c_s=c_s, c_atom=c_atom, c_atompair=c_atompair, **atom_attention_encoder)
    ...
    def forward(self,
        ...
        # Conditioning
        S_I, Z_II = self.diffusion_conditioning(t, f, S_inputs_I.float(), S_trunk_I.float(), Z_trunk_II.float())
        ...
        A_I, Q_skip_L, C_skip_L, P_skip_LL = self.atom_attention_encoder(f, R_noisy_L, S_trunk_I.float(), Z_II)
        ...
        R_update_L = self.atom_attention_decoder(f, A_I.float(), Q_skip_L, C_skip_L, P_skip_LL)
...
class Model(nn.Module):
    ...
    def forward(self, input, n_cycle, no_sync, use_amp=False):
        '''
        ...
        '''
        recycling_input = self.trunk_forward(input, n_cycle=n_cycle, no_sync=no_sync, use_amp=use_amp)
    ...
    def trunk_forward(self, input, n_cycle, no_sync, use_amp=False): 
        ...
        for i_cycle in range(n_cycle):
            with ExitStack() as stack:
                ...
                recycling_input["f"]["msa"] = input["f"]["msa_stack"][i_cycle].to(input["t"].device)
