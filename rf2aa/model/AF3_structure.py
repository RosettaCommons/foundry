...
from rf2aa.model.layers.af3_diffusion_transformer import (
    AtomAttentionEncoderDiffusion,
    AtomTransformer,
    DiffusionTransformer,
)
from rf2aa.model.layers.layer_utils import Transition, linearNoBias
...
class Model(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.diffusion_module = DiffusionModule(
            c_atom=c_atom, c_atompair=c_atompair, c_s=c_s, c_z=c_z, **diffusion_module
        )
    ...
    def forward(self, input, n_cycle, no_sync, use_amp=False):
        ...
        """
        recycling_input = self.trunk_forward(
            input, n_cycle=n_cycle, no_sync=no_sync, use_amp=use_amp
        )
    ...
    def trunk_forward(self, input, n_cycle, no_sync, use_amp=False):
        ...
        for i_cycle in range(n_cycle):
            with ExitStack() as stack:
                ...
                recycling_input["f"]["msa"] = input["f"]["msa_stack"][i_cycle].to(
                    input["t"].device
                )
                recycling_input = self.recycle(**recycling_input)
        return recycling_input

    def pre_recycle(self, f, X_noisy_L, t):
