...
def recycle_step_legacy(
    ddp_model, input, n_cycle, use_amp, nograds=False, force_device=None
):
...
def recycle_step_packed(
    ...
    for i_cycle in range(n_cycle):
        with ExitStack() as stack:
            ...
            rf_outputs, rf_latents = ddp_model(
                input_i, use_checkpoint=use_checkpoint, use_amp=use_amp
            )
    ...
    for i_cycle in range(n_cycle):
        with ExitStack() as stack:
            ...
            rf_outputs, rf_latents = ddp_model(
                ...
                use_checkpoint=use_checkpoint,
                use_amp=use_amp,
                skip_refinement=return_raw,
            )
            output_i = unpack_outputs(rf_outputs, rf_latents, return_raw)
    return output_i


def recycle_step_generic(
    model, input, n_cycle, use_amp, nograds=False, force_device=None
):
    """
...
def run_model_forward_legacy(model, network_input, use_checkpoint=False, device="cpu"):
    ...
    output_i = (None, None, xyz_prev, alpha_prev, mask_recycle)
    input_i = add_recycle_inputs(
        network_input, output_i, 0, gpu, return_raw=False, use_checkpoint=False
    )
    input_i["seq_unmasked"] = input_i["seq_unmasked"].to(gpu)
...
def add_recycle_inputs(
    network_input, output_i, i_cycle, gpu, return_raw=False, use_checkpoint=False
):
    input_i = {}
...
def get_recycle_schedule(max_cycle, n_epochs, n_train, world_size, **kwargs):
    ...
    for i in range(n_epochs):
        recycle_schedule = [
            np.random.randint(1, max_cycle + 1) for _ in range(n_train // world_size)
        ]
...
def get_recycle_schedule_opt(max_cycle, n_epochs, n_train, world_size, **kwargs):
    assert n_train % world_size == 0
    np.random.seed(0)
    recycle_schedule = np.random.randint(
        1, max_cycle + 1, (n_epochs, n_train // world_size)
    )
...
recycle_sampling = {"random": get_random_recycle, "by_batch": get_recycle_schedule_opt}
