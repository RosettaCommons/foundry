...
class ExceptionLogger(contextlib.AbstractContextManager):
    def __exit__(self, exc_type, exc_value, traceback):
        if exc_type:
            print("***Logging exception {}***".format((exc_type, exc_value, traceback)))
...
def assert_equal(got, want):
    ...
    uneq_msg = "    ".join(
        f"idx:{idx}, got:{got}, want:{want}" for idx, got, want in uneq_idx_got_want
    )
...
def apply_to_tensors(e, op):
    ...
    if dataclasses.is_dataclass(e):
        ...
        return type(e)(
            *(
                apply_to_tensors(getattr(e, field.name), op)
...
def minmax(e):
    return apply_to_tensors(
        e, lambda x: (torch.log10(torch.min(x)), torch.log10(torch.max(x)))
    )
...
class TensorMatchOperator(BaseOperator):
    ...
    def _equal_msg(self, got, want):
        ...
        if torch.isclose(
            got, want, equal_nan=True, atol=self.atol, rtol=self.rtol
        ).all():
            return ""
        ...
        unequal_got = got[~is_eq]
        unequal_want = want[~is_eq]
        uneq_idx_got_want = list(zip(unequal_idx.tolist(), unequal_got, unequal_want))[
            :3
        ]
        uneq_msg = "    ".join(
            f"idx:{idx}, got:{got}, want:{want}" for idx, got, want in uneq_idx_got_want
