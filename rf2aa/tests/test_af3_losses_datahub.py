...
from rf2aa.loss.af3_losses import DiffusionLoss
...
def test_weighted_rigid_align(pdb_dataset):
    ...
    for sample in tqdm(data_loader):
        ...
        assert torch.allclose(
            X_align_L[is_resolved], 
            loss_input["X_gt_L"][is_resolved], atol=1e-4), \
...
def test_diffusion_loss(pdb_dataset):
    ...
    for sample in tqdm(data_loader):
        ...
        assert torch.allclose(diffusion_loss, torch.tensor(0.0)), f"The loss value is not zero when provided the ground truth. got: {diffusion_loss}"

        # rescale the noisy inputs and make sure their loss is 1
        sigma_data = loss.sigma_data
        t = network_input["t"]
        null_pred = (sigma_data**2 / (sigma_data**2 + t**2))[...,None,None] * network_input["X_noisy_L"]
        loss_value, loss_dict = loss(
            network_input,
            {"X_L": null_pred},
            loss_input,
        ...
        assert torch.allclose(diffusion_loss, torch.tensor(1.0), atol=0.5), f"The average loss value is not 1 (within 0.25 tolerance) when provided the noisy inputs. got: {diffusion_loss}"
        assert torch.allclose(diffusion_std_dev, torch.tensor(0.0), atol=0.5), f"The std deviation value is not within 0.2 when provided the noisy inputs. got: {diffusion_std_dev}"
