...
class ConfidenceLoss(nn.Module):
    ...
    def forward(
        ...
        if self.log_statistics or self.rank_loss.use_listnet_loss:
            ...
            plddt_per_structure = unbin_logits(plddt_logit_stack.reshape(plddt_logit_stack.shape[0], -1, plddt_logit_stack.shape[1], ChemData().NHEAVY).float(), self.plddt.max_value, self.plddt.n_bins)
