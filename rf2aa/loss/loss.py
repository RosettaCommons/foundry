...
from rf2aa.kinematics import get_ang, get_dih
...
def get_t(N, Ca, C, eps=1e-4):
    ...
    Rs, Ts = rigid_from_3_points(
        N.view(I * B, L, 3), Ca.view(I * B, L, 3), C.view(I * B, L, 3), eps=eps
    )
...
def calc_str_loss(
    ...
    clamped_loss = (mask[None] * clamped_loss).sum(dim=(1, 2, 3)) / (
        mask.sum() + eps
    )  # (I)
    unclamped_loss = (mask[None] * unclamped_loss).sum(dim=(1, 2, 3)) / (
        mask.sum() + eps
    )  # (I)
...
def compute_general_FAPE(
    X,
    Y,
    atom_mask,
    frames,
    frame_mask,
    frame_atom_mask=None,
    frame_atom_mask_2d=None,
    logit_pae=None,
    logit_pde=None,
    Z=10.0,
    ...
    frames_reindex, frame_mask = mask_unresolved_frames(
        frames, frame_mask, frame_atom_mask
    )
    ...
    xij = torch.einsum(
        "brji,brsj->brsi",
        uX[:, frame_mask[0]],
        X[:, atom_mask[0]][:, None, ...] - X_y[:, frame_mask[0]][:, :, None, ...],
    )
    xij_t = torch.einsum(
        "rji,rsj->rsi",
        uY[frame_mask],
        Y[atom_mask][None, ...] - Y_y[frame_mask][:, None, ...],
    )
    ...
    clamped_loss = (
        (1.0 / Z) * torch.sum(torch.clamp(diff, max=dclamp), dim=(1, 2)) / (N_values)
    )
    ...
    pae_loss = (
        compute_pae_loss(
            ...
            frame_mask,
            atom_mask,
            frame_atom_mask_2d=frame_atom_mask_2d,
        )
    ...
    pde_loss = (
        compute_pde_loss(
            X, Y, logit_pde, atom_mask, frame_atom_mask_2d=frame_atom_mask_2d
        )
...
def calc_BB_bond_geom(
    ...
    CN_loss = (
        bonded
        * is_prot
        * torch.clamp(torch.square(blen_CN_pred - ideal_NC) - sig_len**2, min=0.0)
    )
    n_viol = (CN_loss > 0.0).sum()
    blen_loss = CN_loss.sum() / (n_viol + eps)

    # bond length: P-O
    iP, iO3p, iOP1, iOP2, iC3p = (
        4,
        10,
        3,
        5,
        9,
    )  # DNA BB indices! Should live in chemical?

    blen_OP_pred = length(pred[:, :-1, iO3p], pred[:, 1:, iP]).reshape(
        B, L - 1
    )  # (B, L-1)
    OP_loss = (
        bonded
        * is_NA
        * torch.clamp(torch.square(blen_OP_pred - ideal_OP) - sig_len**2, min=0.0)
    )
    ...
    CACN_loss = (
        bonded
        * is_prot
        * torch.clamp(torch.square(bang_CACN_pred - ideal_CACN) - sig_ang**2, min=0.0)
    )
    CNCA_loss = (
        bonded
        * is_prot
        * torch.clamp(torch.square(bang_CNCA_pred - ideal_CNCA) - sig_ang**2, min=0.0)
    )
    ...
    bang_O1PO_pred = cosangle(
        pred[:, :-1, iO3p], pred[:, 1:, iP], pred[:, 1:, iOP1]
    ).reshape(B, L - 1)
    bang_O2PO_pred = cosangle(
        pred[:, :-1, iO3p], pred[:, 1:, iP], pred[:, 1:, iOP2]
    ).reshape(B, L - 1)
    bang_OPC_pred = cosangle(
        pred[:, :-1, iC3p], pred[:, :-1, iO3p], pred[:, 1:, iP]
    ).reshape(B, L - 1)
    O1PO_loss = (
        bonded
        * is_NA
        * torch.clamp(torch.square(bang_O1PO_pred - ideal_OPO) - sig_ang**2, min=0.0)
    )
    O2PO_loss = (
        bonded
        * is_NA
        * torch.clamp(torch.square(bang_O2PO_pred - ideal_OPO) - sig_ang**2, min=0.0)
    )
    OPC_loss = (
        bonded
        * is_NA
        * torch.clamp(torch.square(bang_OPC_pred - ideal_OPC) - sig_ang**2, min=0.0)
    )
...
def calc_atom_bond_loss(
    ...
    j_ca = torch.argmax(
        torch.arange(N, device=seq.device)[None, :] * (bond_feats[0, j] == 1), dim=1
    )
    resids = torch.stack(
        [i, i, j, j_ca], dim=-1
    )  # ASSUME: since atoms are at end, i,j pairs are all res,atom
    ...
    blen_CN_pred = length(
        pred[:, resids[:, 1], atomids[:, 1]], pred[:, resids[:, 2], atomids[:, 2]]
    )
    CN_loss = torch.clamp(torch.square(blen_CN_pred - ideal_NC) - sig_len**2, min=0.0)
    n_viol = (CN_loss > 0.0).sum()
    bond_dist_loss += CN_loss.sum() / (n_viol + eps)

    bang_CACN_pred = cosangle(
        pred[:, resids[:, 0], atomids[:, 0]],
        pred[:, resids[:, 1], atomids[:, 1]],
        pred[:, resids[:, 2], atomids[:, 2]],
    )
    bang_CNCA_pred = cosangle(
        pred[:, resids[:, 1], atomids[:, 1]],
        pred[:, resids[:, 2], atomids[:, 2]],
        pred[:, resids[:, 3], atomids[:, 3]],
    )
    CACN_loss = torch.clamp(
        torch.square(bang_CACN_pred - ideal_CACN) - sig_ang**2, min=0.0
    )
    CNCA_loss = torch.clamp(
    ...
    if paths:
        ...
        nat_dist = torch.sum(
            torch.square(true[:, paths[:, 0], 1] - true[:, paths[:, 2], 1]), dim=-1
        )
        pred_dist = torch.sum(
            torch.square(pred[:, paths[:, 0], 1] - pred[:, paths[:, 2], 1]), dim=-1
        )
    ...
    if rigid_groups != None:
        ...
        pred_dist = torch.sum(
            torch.square(
                pred[:, rigid_groups[:, 0], 1] - pred[:, rigid_groups[:, 1], 1]
            ),
            dim=-1,
        )
...
def calc_BB_bond_geom_grads(
    ...
    Ebond = calc_BB_bond_geom(
        seq, pred, idx, eps, ideal_NC, ideal_CACN, ideal_CNCA, sig_len, sig_ang
    )
...
def calc_chiral_grads(xyz, chirals):
    ...
    if l.item() == 0.0:
        return (
            torch.zeros(xyz.shape, device=xyz.device),
        )  # autograd returns a tuple..
