...
from kinematics import c6d_to_bins2, get_init_xyz, xyz_to_c6d, xyz_to_t2d
from loss import *
...
LOAD_PARAM = {"shuffle": False, "num_workers": 4, "pin_memory": True}
...
class Predictor:
    ...
    def run_prediction(
        ...
        with torch.no_grad():
            ...
            msa_prev = None
            pair_prev = None
            alpha_prev = torch.zeros((1, L, 10, 2)).to(
                self.device, non_blocking=True
            )  # fd we could get this from the template...
            state_prev = None
            ...
            for i_cycle in range(MAX_CYCLE):
                with torch.cuda.amp.autocast(True):
                    ...
                    logit_aa_s = logit_aa_s.reshape(B, -1, N, L)[:, :, 0].permute(
                        0, 2, 1
                    )
            ...
            natRs_all, _n0 = self.compute_allatom_coords(
                seq, true_crds[..., :3, :], true_tors
            )
            natRs_all_alt, _n1 = self.compute_allatom_coords(
            ...
            true_lddt = calc_allatom_lddt(
                best_xyz.unsqueeze(0), nat_symm, idx_pdb, atom_mask
            )
            ...
            print(
                tag[0],
                tag[1],
                true_lddt.mean().cpu().numpy(),
                ljE.cpu().numpy(),
                cbE.cpu().numpy(),
            )
            self.write_pdb(
                seq[0], best_xyz, Bfacts=best_lddt[0], prefix="preds/%s_pred" % (tag[0])
            )
...
def get_args():
    ...
    parser.add_argument(
        "-i", default=1, required=False, type=int, help="parallelize i of j"
    )
    parser.add_argument(
        "-j", default=1, required=False, type=int, help="parallelize i of j"
    )
...
if __name__ == "__main__":
    ...
    with open(LOADER_PARAMS["FB_LIST"], "r") as f:
        ...
        next(reader)
        rows = [
            [r[0], r[2], int(r[3]), len(r[-1].strip())]
            for r in reader
            if float(r[1]) > 85.0 and len(r[-1].strip()) > 200
        ]
    fb = {}
    for r in rows:
        if r[2] in fb.keys():
    ...
    for i, (id_i, key_i) in enumerate(fb.items()):
        ...
        seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
            msa, ins, LOADER_PARAMS, p_mask=0.0
        )
        ...
        f1d_t = torch.nn.functional.one_hot(
            torch.full((1, l), 20).long(), num_classes=21
        ).float()  # all gaps
