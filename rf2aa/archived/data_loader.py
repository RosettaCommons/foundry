...
from rf2aa.data.chain_crop import (
    get_complex_crop,
    get_crop,
    get_discontiguous_crop,
    get_na_crop,
    get_spatial_crop,
)
from rf2aa.data.parsers import (
    get_dislf,
    parse_a3m,
    parse_fasta_if_exists,
    parse_mixed_fasta,
    parse_mol,
    parse_pdb,
)
...
from rf2aa.util import (
    atomize_discontiguous_residues,
    bond_feats_from_Ls,
    center_and_realign_missing,
    cif_ligand_to_xyz,
    expand_xyz_sm_to_ntotal,
    get_atom_frames,
    get_atom_template_indices,
    get_bond_feats,
...
try:
    ...
    logger.warning(
        "Failed to import cluster_factory from rf2aa.data.cluster_dataset: if you are rebuilding the dataset .pkl expect failure: "
        + repr(e)
    )
...
def blank_template(n_tmpl, L, random_noise=5.0):
    xyz = (
        ChemData().INIT_CRDS.reshape(1, 1, ChemData().NTOTAL, 3).repeat(n_tmpl, L, 1, 1)
        + torch.rand(n_tmpl, L, 1, 3) * random_noise
        - random_noise / 2
    )
    t1d = torch.nn.functional.one_hot(
        torch.full((n_tmpl, L), 20).long(), num_classes=ChemData().NAATOKENS - 1
    ).float()  # all gaps
...
def TemplFeaturize(
    ...
    if (ntplt < 1) or (
        npick < 1
    ):  # no templates in hhsearch file or not want to use templ
    ...
    xyz = (
        ChemData()
        .INIT_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
        .repeat(npick_global, qlen, 1, 1)
        + torch.rand(1, qlen, 1, 3) * random_noise
    )
    mask_t = torch.full(
        (npick_global, qlen, ChemData().NTOTAL), False
    ...
    t1d = torch.nn.functional.one_hot(
        t1d, num_classes=ChemData().NAATOKENS - 1
    ).float()  # (no mask token)
    t1d = torch.cat((t1d, t1d_val[..., None]), dim=-1)

    tplt_ids = np.array(tplt["ids"])[
        sample
    ].flatten()  # np.array of chain ids (ordered)
...
def merge_hetero_templates(xyz_t_prot, f1d_t_prot, mask_t_prot, tplt_ids, Ls_prot):
    ...
    tplt_ids_out = np.full(
        (N_tmpl_tot), "", dtype=object
    )  # rk bad practice.. should fix
    i_tmpl = 0
    i_res = 0
...
def spoof_template(xyz, seq, mask, is_motif=None, template_conf=1, random_noise=5):
    ...
    xyz_t = (
        ChemData().INIT_CRDS.reshape(1, 1, ChemData().NTOTAL, 3).repeat(1, L, 1, 1)
        + torch.rand(1, L, 1, 3) * random_noise
    )

    t1d = torch.cat(
        (
            torch.nn.functional.one_hot(
                torch.full((1, L), 20).long(), num_classes=ChemData().NAATOKENS - 1
            ).float(),  # all gaps (no mask token)
            torch.zeros((1, L, 1)).float(),
        ),
        -1,
    )  # (1, L_protein + L_sm, NAATOKENS)
    ...
    t1d[0, is_motif] = torch.cat(
        (
            torch.nn.functional.one_hot(
                seq[is_motif], num_classes=ChemData().NAATOKENS - 1
            ).float(),
            torch.full((len(is_motif), 1), template_conf).float(),
        ),
        -1,
    )  # (1, L_protein, NAATOKENS)
    mask_t[0, is_motif, :14] = mask[is_motif, :14]
    return xyz_t, t1d, mask_t


def generate_sm_template_feats(
    tplt_ids, resnames, akeys, Ls_sm, chid2smpartners, params
):
    """
    ...
    """
    sim, names = load_tanimoto_sim_matrix(
        base_path=params["SM_COMPL_DIR"]
    )  # could load this earlier...
    ...
    for chid in tplt_ids:
        ...
        chains, _, covale, _ = pickle.load(
            gzip.open(params["MOL_DIR"] + f"/{chid[1:3]}/{chid.split('_')[0]}.pkl.gz")
        )
        ...
        template_partner_names = [
            ligand[0][2]
            for ligand in template_partners
            if ligand[0][2] in names and ligand[0][2] is not None
        ]

        if (
            len(template_partner_names) == 0
        ):  # this is the case where all the template partners are metals or sugars
            xyz_t, f1d_t, mask_t, _ = blank_template(1, sum(Ls_sm))
            xyz_t_all_template.append(xyz_t)
            f1d_t_all_template.append(f1d_t)
            mask_t_all_template.append(mask_t)
        ...
        for i, (lig_name, L) in enumerate(zip(resnames, Ls_sm)):
            ...
            if lig_name in template_partner_names:
                ...
                input_akeys = None
                if (
                    template_partner_idx is None
                ):  # case where more ligands in query than template
            ...
            if input_akeys is not None:
                input_akeys = [
                    tuple(list(ligand[0][:2]) + list(key[2:])) for key in input_akeys
                ]

            try:
                xyz_, occ_, msa_, chid_, akeys_ = cif_ligand_to_xyz(
                    lig_atoms, asmb_xfs, ch2xf, input_akeys=input_akeys
                )
            except Exception as e:
            ...
            if (
                input_akeys is None or len(akeys_) != len(input_akeys)
            ):  # length of template is different from ground truth, need to remake tensors to match
                ligand_com = torch.mean(xyz_, dim=0)
                xyz_ = torch.zeros((L, 3))
                xyz_[:] = ligand_com + torch.rand(
                    3
                )  # add a little noise to avoid learning templates are at the com
                ...
                assert input_akeys == akeys_, (
                    "if provided input akeys, output akeys must match"
                )
            ...
            xyz_t, f1d_t, mask_t = spoof_template(
                xyz_sm, msa_.long(), mask_sm, template_conf=max_tanimoto
            )
            xyz_t_all_lig.append(xyz_t)
            f1d_t_all_lig.append(f1d_t)
            mask_t_all_lig.append(mask_t)
...
def params_match_pickle(
    ...
    match_keys: List[str] = [
        "ligands_to_remove",
        "weight_sm_compl_by_seq_len",
        "sm_compl_cluster_method",
    ],
) -> bool:
    """
...
def get_train_valid_set(
    ...
    def _apply_date_res_cutoffs(df):
        ...
        return df[
            (df.RESOLUTION <= loader_params["RESCUT"])
            & (
                df.DEPOSITION.apply(lambda x: parser.parse(x))
                <= parser.parse(loader_params["DATCUT"])
            )
        ]
    ...
    def _get_IDs_weights(df):
        ...
        weights = (1 / 512.0) * np.clip(tmp_df.LEN_EXIST.values, 256, 512)
    ...
    val_neg_ids = set(
        [int(l) + NEG_CLUSID_OFFSET for l in open(loader_params["VAL_NEG"]).readlines()]
    )
    val_rna_pdb_ids = set(
        [l.rstrip() for l in open(loader_params["VAL_RNA"]).readlines()]
    )
    val_dna_pdb_ids = set(
        [l.rstrip() for l in open(loader_params["VAL_DNA"]).readlines()]
    )
    ...
    train_dict["pdb"] = pdb[
        (~pdb.CLUSTER.isin(val_pdb_ids)) & (~pdb.CLUSTER.isin(test_sm_ids))
    ]
    valid_dict["pdb"] = pdb[
        pdb.CLUSTER.isin(val_pdb_ids) & (~pdb.CLUSTER.isin(test_sm_ids))
    ]
    ...
    valid_dict["dslf"] = dslf.merge(
        tmp_df[["CHAINID", "HASH", "CLUSTER"]],
        left_on="CHAIN_A",
        right_on="CHAINID",
        how="right",
    )
    ...
    compl["LEN_EXIST"] = compl["LEN"].apply(
        lambda x: sum(x)
    )  # total length, for computing weights

    valid_dict["compl"] = compl[compl.CLUSTER.isin(val_compl_ids)]
    train_dict["compl"] = compl[
        (~compl.CLUSTER.isin(val_compl_ids))
        & (~compl.HASH_A.isin(val_hash))
        & (~compl.HASH_B.isin(val_hash))
    ]
    train_ID_dict["compl"], weights_dict["compl"] = _get_IDs_weights(
        train_dict["compl"]
    )
    ...
    train_dict["neg_compl"] = neg[
        (~neg.CLUSTER.isin(val_neg_ids))
        & (~neg.HASH_A.isin(val_hash))
        & (~neg.HASH_B.isin(val_hash))
    ]
    train_ID_dict["neg_compl"], weights_dict["neg_compl"] = _get_IDs_weights(
        train_dict["neg_compl"]
    )
    valid_ID_dict["neg_compl"] = (
        valid_dict["neg_compl"].CLUSTER.drop_duplicates().values
    )
    ...
    train_ID_dict["na_compl"], weights_dict["na_compl"] = _get_IDs_weights(
        train_dict["na_compl"]
    )
    ...
    train_ID_dict["neg_na_compl"], weights_dict["neg_na_compl"] = _get_IDs_weights(
        train_dict["neg_na_compl"]
    )
    valid_ID_dict["neg_na_compl"] = (
        valid_dict["neg_na_compl"].CLUSTER.drop_duplicates().values
    )
    ...
    train_ID_dict["distil_tf"], weights_dict["distil_tf"] = _get_IDs_weights(
        train_dict["distil_tf"]
    )
    valid_ID_dict["distil_tf"] = (
        valid_dict["distil_tf"].CLUSTER.drop_duplicates().values
    )
    ...
    train_ID_dict["neg_tf"], weights_dict["neg_tf"] = _get_IDs_weights(
        train_dict["neg_tf"]
    )
    ...
    in_val = rna["CHAINID"].apply(
        lambda x: any([y in val_rna_pdb_ids for y in x.split(":")])
    )
    ...
    in_val = dna["CHAINID"].apply(
        lambda x: any([y in val_dna_pdb_ids for y in x.split(":")])
    )
    ...
    def _prep_sm_compl_data(df):
        ...
        if loader_params.get("weight_sm_compl_by_seq_len", True):
            seq_len_factor = (1 / 512.0) * np.clip(
                df.LEN_EXIST, 256, 512
            )  # standard seq length weighting
            df.loc[:, "WEIGHT"] = (
                seq_len_factor  # can potentially include other factors (ligand cluster size, etc)
            )
        else:
            df["WEIGHT"] = 1.0
    ...
    # protein / small molecule complexes
    df_sm = _load_df(
        loader_params["SM_LIST"], eval_cols=["COVALENT", "LIGAND", "LIGXF", "PARTNERS"]
    )
    ...
    chid2smpartners = (
        df_sm.groupby("CHAINID").agg(lambda x: [val for val in x])["LIGAND"].to_dict()
    )
    ...
    sm = _load_df(
        loader_params["CSD_LIST"],
        pad_hash=False,
        eval_cols=["sim", "sim_valid", "sim_test"],
    )
    ...
    sm = sm.drop(
        ["sim", "sim_test", "sim_valid"], axis=1
    )  # drop these memory-intensive columns
...
def find_msa_hashes(protein_chain_info, params):
    ...
    for item1, item2 in itertools.permutations(protein_chain_info, 2):
        ...
        if (
            ...
            and item1["query_taxid"] == item2["query_taxid"]
        ):  # different hashes but same tax id, means there is a pMSA generated
            msaA_id = item1["hash"]
            msaB_id = item2["hash"]
                    ...
                    msas_to_load.append(
                        {
                            ...
                            "seq_range": (
                                item2["len"],
                                item1["len"] + item2["len"],
                            ),  # store sequence indices to only pull out second chain
                            "paired": True,
                        }
                    )
    ...
    unpaired_items = [
        item for item in protein_chain_info if item not in updated_protein_chain_info
    ]
    unpaired_msas = [
        {
            "path": params["PDB_DIR"]
        ...
        for info in unpaired_items
    ]
    updated_protein_chain_info.extend(
        unpaired_items
    )  # maps the order of the chains to the order of loaded MSAs so coordinates and msa match
    msas_to_load.extend(
        unpaired_msas
    )  # msas_to_load will be the same length as updated_protein_chain_info
    ...
    try:
        original_pci_order = [
            updated_protein_chain_info.index(info) for info in protein_chain_info
        ]
    except Exception as e:
        print(
            f"ERROR: there is a protein chain that was supposed to be loaded that was not: input chains: {str(protein_chain_info)}   output_chains: {str(updated_protein_chain_info)}"
        )
        raise e
    msas_to_load = [msas_to_load[i] for i in original_pci_order]

    assert len(protein_chain_info) == len(msas_to_load), (
        f"not all protein chains had corresponding MSAs: {str(protein_chain_info)} "
    )
    return msas_to_load
...
def get_assembly_msa(protein_chain_info, params):
    ...
    if len(msa_hashes) == 0:
        raise NotImplementedError(
            f"No MSAs were found for these protein chains {str(protein_chain_info)}"
        )
...
def merge_a3m_hetero(a3mA, a3mB, L_s):
    ...
    if a3mA["msa"].shape[0] > 1:
        extra_A = torch.nn.functional.pad(
            a3mA["msa"][1:], (0, sum(L_s[1:])), "constant", 20
        )  # pad gaps
        msa.append(extra_A)
    ...
    if a3mA["ins"].shape[0] > 1:
        extra_A = torch.nn.functional.pad(
            a3mA["ins"][1:], (0, sum(L_s[1:])), "constant", 0
        )  # pad gaps
        ins.append(extra_A)
    ...
    if "taxid" in a3mA and "taxid" in a3mB:
        a3m["taxid"] = np.concatenate(
            [np.array(a3mA["taxid"]), np.array(a3mB["taxid"])[1:]]
        )
    ...
    for i in range(1, len(a3m_list)):
        ...
        if a3mB["hash"] in seen or len(pair_taxIDs) < 5:  # homomer/not enough pairs
            ...
            for pair in pair_taxIDs:
                ...
                msaApair = torch.argmin(
                    torch.sum(msaA[pair_a3mA, :] == msaA[0, :], axis=-1)
                )
                msaBpair = torch.argmin(
                    torch.sum(msaB[pair_a3mB, :] == msaB[0, :], axis=-1)
                )
                final_pairsA.append(pair_a3mA[msaApair])
                final_pairsB.append(pair_a3mB[msaBpair])
            paired_msaB = torch.full(
                (msaA.shape[0], L_s[i]), 20
            ).long()  # (N_seq_A, L_B)
            paired_msaB[final_pairsA] = msaB[final_pairsB]
            msaA = torch.cat([msaA, paired_msaB], dim=1)
            insA = torch.zeros_like(
                msaA
            )  # paired MSAs in our dataset dont have insertions
...
def get_bond_distances(bond_feats):
    ...
    dist_matrix = scipy.sparse.csgraph.shortest_path(
        atom_bonds.long().numpy(), directed=False
    )
...
def featurize_single_chain(
    ...
    xyz_t, f1d_t, mask_t, _ = TemplFeaturize(
        tplt,
        msa.shape[1],
        params,
        npick=ntempl,
    ...
    if disulf_crop:
        ...
        xyz = torch.full(
            (N_symmetry, protein_L + sm_L, ChemData().NTOTAL, 3), np.nan
        ).float()
        mask = torch.full(xyz.shape[:-1], False).bool()
        xyz[:, :protein_L, :nprotatoms, :] = xyz_prot.expand(
            N_symmetry, protein_L, nprotatoms, 3
        )
        xyz[:, protein_L:, 1, :] = xyz_atomize_all
        mask[:, :protein_L, :nprotatoms] = mask_prot.expand(
            N_symmetry, protein_L, nprotatoms
        )
        mask[:, protein_L:, 1] = mask_atomize_all

        # generate (empty) template for atoms
        tplt_sm = {"ids": []}
        xyz_t_sm, f1d_t_sm, mask_t_sm, _ = TemplFeaturize(
            tplt_sm,
            xyz_atomize_all.shape[1],
            params,
            offset=0,
            npick=0,
            pick_top=pick_top,
        ...
        a3m_sm = {
            "msa": seq_atomize_all.unsqueeze(0),
            "ins": ins_atomize_all.unsqueeze(0),
        }
        ...
        seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
            msa, ins, params, fixbb=fixbb, **msa_featurization_kwargs
        )
...
def featurize_homo(
    ...
    msa, ins = merge_a3m_homo(
        msa_orig, ins_orig, 2
    )  # make unpaired alignments, for training, we always use two chains
    seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
        msa, ins, params, L_s=[L, L]
    )
    ...
    for i_int, interface in enumerate(interfaces):
        ...
        xyzA = (
            torch.einsum("ij,raj->rai", xformA[:3, :3], pdbA["xyz"])
            + xformA[:3, 3][None, None, :]
        )
        xyzB = (
            torch.einsum("ij,raj->rai", xformB[:3, :3], pdbB["xyz"])
            + xformB[:3, 3][None, None, :]
        )
    ...
    if cropsub * L > params["CROP"]:
        ...
        crop_idx = get_crop(
            L,
            mask[0, :L],
            msa_seed_orig.device,
            params["CROP"] // cropsub,
            unclamp=False,
        )  # cropped region for first copy
...
def loader_pdb(
    ...
    pdb = torch.load(
        params["PDB_DIR"] + "/torch/pdb/" + pdb_chain[1:3] + "/" + pdb_chain + ".pt"
    )
    a3m = get_msa(
        params["PDB_DIR"] + "/a3m/" + pdb_hash[:3] + "/" + pdb_hash + ".a3m.gz",
        pdb_hash,
    )
    tplt = torch.load(
        params["PDB_DIR"] + "/torch/hhr/" + pdb_hash[:3] + "/" + pdb_hash + ".pt"
    )
    ...
    if pdb_chain in homo["CHAIN_A"].values and np.random.rand() < p_homo_cut:
        ...
        interfaces = homo[homo["CHAIN_A"] == pdb_chain].to_dict(
            orient="records"
        )  # list of dicts
        feats = featurize_homo(
            msa,
            ins,
            tplt,
            pdb,
    ...
    feats = featurize_single_chain(
        ...
        pick_top=pick_top,
        fixbb=fixbb,
        p_short_crop=p_short_crop,
        p_dslf_crop=p_dslf_crop,
    )
    return feats + (
        "monomer",
        item,
...
def loader_fb(
    item, params, unclamp=False, p_short_crop=0.0, p_dslf_crop=0.0, fixbb=False
):
    # loads sequence/structure/plddt information
    pdb_chain, hashstr = item["CHAINID"], item["HASH"]
    a3m = get_msa(
        os.path.join(
            params["FB_DIR"], "a3m", hashstr[:2], hashstr[2:], pdb_chain + ".a3m.gz"
        ),
        pdb_chain,
    )
    pdb = get_pdb(
        os.path.join(
            params["FB_DIR"], "pdb", hashstr[:2], hashstr[2:], pdb_chain + ".pdb"
        ),
        os.path.join(
            params["FB_DIR"], "pdb", hashstr[:2], hashstr[2:], pdb_chain + ".plddt.npy"
        ),
        pdb_chain,
        params["PLDDTCUT"],
        params["SCCUT"],
    ...
    if disulf_crop:
        ...
        tplt_sm = {"ids": []}
        xyz_t_sm, f1d_t_sm, mask_t_sm, _ = TemplFeaturize(
            tplt_sm, xyz_atomize_all.shape[1], params, offset=0, npick=0
        )
        ntempl = xyz_t_prot.shape[0]
        xyz_t = torch.cat((xyz_t_prot, xyz_t_sm.repeat(ntempl, 1, 1, 1)), dim=1)
        f1d_t = torch.cat((f1d_t_prot, f1d_t_sm.repeat(ntempl, 1, 1)), dim=1)
...
def loader_complex(
    item, params, negative=False, pick_top=True, random_noise=5.0, fixbb=False
):
    pdb_pair, pMSA_hash, L_s, taxID = (
        item["CHAINID"],
        item["HASH"],
        item["LEN"],
        item["TAXONOMY"],
    ...
    if (
        len(set(taxID.split(":"))) == 1
    ):  # two proteins have same taxID -- use paired MSA
        # read pMSA
        if negative:
            pMSA_fn = (
                params["COMPL_DIR"]
    ...
    if len(msa) > params["BLOCKCUT"]:
        msa, ins = MSABlockDeletion(msa, ins)
    seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
        msa, ins, params, L_s=L_s, fixbb=fixbb
    )
    ...
    xyz_t_A, f1d_t_A, mask_t_A, _ = TemplFeaturize(
        tpltA,
        L_s[0],
        params,
        offset=0,
        npick=ntemplA,
        npick_global=max(1, max(ntemplA, ntemplB)),
        pick_top=pick_top,
        random_noise=random_noise,
    )
    xyz_t_B, f1d_t_B, mask_t_B, _ = TemplFeaturize(
        tpltB,
        L_s[1],
        params,
        offset=0,
    ...
    xyz_t = torch.cat(
        (xyz_t_A, random_rot_trans(xyz_t_B)), dim=1
    )  # (T, L1+L2, natm, 3)
    ...
    pdbA = torch.load(
        params["PDB_DIR"] + "/torch/pdb/" + pdbA_id[1:3] + "/" + pdbA_id + ".pt"
    )
    pdbB = torch.load(
        params["PDB_DIR"] + "/torch/pdb/" + pdbB_id[1:3] + "/" + pdbB_id + ".pt"
    )
    ...
    if not negative:
        ...
        meta = torch.load(
            params["PDB_DIR"] + "/torch/pdb/" + pdbid[1:3] + "/" + pdbid + ".pt"
        )
    ...
    if sum(L_s) > params["CROP"]:
        if negative:
            ...
            sel = get_spatial_crop(
                xyz, mask, torch.arange(sum(L_s)), L_s, params, pdb_pair
            )
        #
        seq = seq[:, sel]
...
def loader_na_complex(
    ...
    # read protein MSA
    a3mA = get_msa(
        params["PDB_DIR"] + "/a3m/" + msa_id[:3] + "/" + msa_id + ".a3m.gz",
        msa_id,
        maxseq=5000,
    )
    ...
    if len(pdb_ids) == 2:
        ...
        filenameB = (
            params["NA_DIR"] + "/torch/" + pdb_ids[1][1:3] + "/" + pdb_ids[1] + ".pt"
        )
        ...
        msaB, insB = parse_fasta_if_exists(
            pdbB[0]["seq"],
            params["NA_DIR"] + "/torch/" + pdb_ids[1][1:3] + "/" + pdb_ids[1] + ".afa",
            maxseq=5000,
            rmsa_alphabet=True,
        )
    ...
    # protein + NA duplex
        ...
        filenameB2 = (
            params["NA_DIR"] + "/torch/" + pdb_ids[2][1:3] + "/" + pdb_ids[2] + ".pt"
        )
        ...
        msaB2, insB2 = parse_fasta_if_exists(
            pdbB[1]["seq"],
            params["NA_DIR"] + "/torch/" + pdb_ids[2][1:3] + "/" + pdb_ids[2] + ".afa",
            maxseq=5000,
            rmsa_alphabet=True,
        )
    ...
    # homodimer + NA duplex
        ...
        filenameB2 = (
            params["NA_DIR"] + "/torch/" + pdb_ids[3][1:3] + "/" + pdb_ids[3] + ".pt"
        )
        if os.path.exists(filenameB1 + ".v3"):
            filenameB1 = filenameB1 + ".v3"
        if os.path.exists(filenameB2 + ".v3"):
        ...
        msaB2, insB2 = parse_fasta_if_exists(
            pdbB[1]["seq"],
            params["NA_DIR"] + "/torch/" + pdb_ids[3][1:3] + "/" + pdb_ids[3] + ".afa",
            maxseq=5000,
            rmsa_alphabet=True,
        )
        a3mB1 = {"msa": torch.from_numpy(msaB1), "ins": torch.from_numpy(insB1)}
    ...
    if not negative and padding:
        ...
        insB1 = torch.cat(
            (torch.zeros_like(lseq1), a3mB1["ins"], torch.zeros_like(rseq1)), dim=1
        )
        insB2 = torch.cat(
            (torch.zeros_like(lseq2), a3mB2["ins"], torch.zeros_like(rseq2)), dim=1
        )
        ...
        # pad PDB
        pdbB[0]["xyz"] = torch.nn.functional.pad(
            pdbB[0]["xyz"], (0, 0, 0, 0, lpad, rpad), "constant", 0.0
        )
        pdbB[0]["mask"] = torch.nn.functional.pad(
            pdbB[0]["mask"], (0, 0, lpad, rpad), "constant", False
        )
        pdbB[1]["xyz"] = torch.nn.functional.pad(
            pdbB[1]["xyz"], (0, 0, 0, 0, rpad, lpad), "constant", 0.0
        )
        pdbB[1]["mask"] = torch.nn.functional.pad(
            pdbB[1]["mask"], (0, 0, rpad, lpad), "constant", False
        )

    # rewrite seq if negative
    if negative:
        alphabet = np.array(list("ARNDCQEGHILKMFPSTWYV-Xacgtxbdhuy"), dtype="|S1").view(
            np.uint8
        )
    ...
    # read template info
    tpltA = torch.load(
        params["PDB_DIR"] + "/torch/hhr/" + msa_id[:3] + "/" + msa_id + ".pt"
    )
    ntempl = np.random.randint(params["MINTPLT"], params["MAXTPLT"] - 1)
    if len(pdb_ids) == 4:
        if ntempl < 1:
            ...
            xyz_t_single, f1d_t_single, mask_t_single, _ = TemplFeaturize(
                tpltA,
                Ls[0],
                params,
                npick=ntempl,
            ...
            # duplicate
            xyz_t = torch.cat(
                (xyz_t_single, random_rot_trans(xyz_t_single)), dim=1
            )  # (ntempl, 2*L, natm, 3)
            f1d_t = torch.cat((f1d_t_single, f1d_t_single), dim=1)  # (ntempl, 2*L, 21)
            mask_t = torch.cat(
                (mask_t_single, mask_t_single), dim=1
            )  # (ntempl, 2*L, natm)
        ...
        xyz_t = torch.cat(
            (
                xyz_t,
                ChemData()
                .INIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
                .repeat(ntmpl, nNA, 1, 1)
                + torch.rand(ntmpl, nNA, 1, 3) * random_noise,
            ),
            dim=1,
        )
        f1d_t = torch.cat(
            (
                f1d_t,
                torch.nn.functional.one_hot(
                    torch.full((ntmpl, nNA), 20).long(),
                    num_classes=ChemData().NAATOKENS,
                ).float(),
            ),
            dim=1,
        )  # add extra class for 0 confidence
        mask_t = torch.cat(
        ...
        xyz_t[:, Ls[0] :] = (
            ChemData()
            .INIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
            .repeat(1, sum(Ls[1:]), 1, 1)
            + torch.rand(1, sum(Ls[1:]), 1, 3) * random_noise
        )
        NAstart = Ls[0]
    ...
    if np.random.rand() <= native_NA_frac:
        ...
        xyz_t_B = (
            ...
            .INIT_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
            .repeat(1, sum(Ls), 1, 1)
            + torch.rand(1, sum(Ls), 1, 3) * random_noise
        )
        ...
        seq_t_B = torch.cat(
            (torch.full((1, NAstart), 20).long(), a3mB["msa"][0:1]), dim=1
        )
        seq_t_B[seq_t_B > 21] -= 1  # remove mask token
        f1d_t_B = torch.nn.functional.one_hot(
            seq_t_B, num_classes=ChemData().NAATOKENS - 1
        ).float()
        conf_B = torch.cat(
            (
    ...
    if len(msa) > params["BLOCKCUT"]:
        msa, ins = MSABlockDeletion(msa, ins)
    seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
        msa, ins, params, p_mask=params["p_msa_mask"], L_s=Ls, fixbb=fixbb
    )
    ...
    if len(pdb_ids) == 2:
        ...
        if NMDLS == 4:  # B & C are identical
            ...
            xyz[2:, NAstart:, :23] = torch.cat((pdbB[1]["xyz"], pdbB[0]["xyz"]), dim=0)[
                None, ...
            ]
            mask[2, :NAstart, :14] = torch.cat(
                (pdbA[0]["mask"], pdbA[1]["mask"]), dim=0
            )
            mask[3, :NAstart, :14] = torch.cat(
                (pdbA[1]["mask"], pdbA[0]["mask"]), dim=0
            )
            mask[2:, NAstart:, :23] = torch.cat(
                (pdbB[1]["mask"], pdbB[0]["mask"]), dim=0
            )[None, ...]
    xyz = torch.nan_to_num(xyz)
...
def loader_tf_complex(
    ...
    a3mA = get_msa(
        params["TF_DIR"] + f"/a3m_v2/{gene_id[:2]}/{gene_id}_aligned_domain.a3m", HASH
    )
    ...
    alphabet = np.array(list("00000000000000000000-0ACGTD00000"), dtype="|S1").view(
        np.uint8
    )
    ...
    xyz_t[:, LA:] = (
        ChemData().INIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3).repeat(1, LB, 1, 1)
        + torch.rand(1, LB, 1, 3) * random_noise
    )
    ...
    if len(msa) > params["BLOCKCUT"]:
        msa, ins = MSABlockDeletion(msa, ins)
    seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
        msa, ins, params, L_s=Ls, fixbb=fixbb
    )
    ...
    xyz[:, is_NA] = (
        ChemData()
        .NIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
        .repeat(1, is_NA.sum(), 1, 1)
        + torch.rand(1, is_NA.sum(), 1, 3) * random_noise
    )
    is_prot = ~is_NA
    xyz[:, is_prot] = (
        ChemData()
        .INIT_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
        .repeat(1, is_prot.sum(), 1, 1)
        + torch.rand(1, is_prot.sum(), 1, 3) * random_noise
    )
...
def loader_distil_tf(
    ...
    xyz_t[:, NAstart:] = (
        ChemData()
        .INIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3)
        .repeat(1, sum(Ls[-2:]), 1, 1)
        + torch.rand(1, sum(Ls[-2:]), 1, 3) * random_noise
    )
    ...
    if sum(Ls) > CROP:
        sel = get_na_crop(
            seq[0], xyz, mask, torch.arange(sum(Ls)), Ls, params, negative=False
        )
...
def choose_matching_seq(seqs, pos_seq):
    ...
    for l in range(min(L, 10), 2, -1):
        ...
        idxs = tuple(
            i for i in range(L + 1 - l) if tuple(pos_msa[0, i : i + l]) == pos_subseq
        )
        idxs += tuple(
            i + L
            for i in range(L + 1 - l)
            if tuple(pos_rc_msa[0, i : i + l]) == pos_subseq
        )
...
def loader_dna_rna(item, params, random_noise=5.0, fixbb=False):
    ...
    filenameA = (
        params["NA_DIR"] + "/torch/" + pdb_ids[0][1:3] + "/" + pdb_ids[0] + ".pt"
    )
    ...
    msaA, insA = parse_fasta_if_exists(
        pdbA["seq"],
        params["NA_DIR"] + "/torch/" + pdb_ids[0][1:3] + "/" + pdb_ids[0] + ".afa",
        rmsa_alphabet=True,
    )
    ...
    L = sum(Ls)
    xyz_t = (
        ChemData().INIT_NA_CRDS.reshape(1, 1, ChemData().NTOTAL, 3).repeat(1, L, 1, 1)
        + torch.rand(1, L, 1, 3) * random_noise
    )
...
def loader_atomize_pdb(
    ...
    xyz_t_prot, f1d_t_prot, mask_t_prot, _ = TemplFeaturize(
        tplt,
        len(pdb["xyz"]),
        params,
        offset=0,
    ...
    idx_missing_N = (
        torch.where(~mask_prot[1:, 0])[0] + 1
    )  # residues missing bb N, excluding 1st residue
    idx_missing_C = torch.where(~mask_prot[:-1, 2])[
        0
    ]  # residues missing bb C, excluding last residue
    ...
    num_atoms_per_res = (
        ChemData().allatom_mask[msa_prot[0], :14].sum(dim=-1)
    )  # how many atoms should each residue have?
    num_atoms_exist = mask_prot.sum(
        dim=-1
    )  # how many atoms have coords in each residue?
    ...
    if flank + 1 >= can_atomize_idx.shape[0] - (n_res_atomize + flank + 1):
        return featurize_single_chain(
            msa, ins, tplt, pdb, params, random_noise=random_noise
        ) + (
            "atomize_pdb",
            item,
        )
    ...
    if params.get("ATOMIZE_CLUSTER", False) and (
        np.random.rand() < 0.9
    ):  # 10% of time do continuous crop
        res_idxs_to_atomize = get_residue_contacts(
            xyz_prot[can_atomize_idx], can_atomize_idx, n_res_atomize
        )

    if (
        res_idxs_to_atomize is None
    ):  # this is triggered if triple contact fails or if the task is not triple contact
        i_start = torch.randint(
            flank + 1, can_atomize_idx.shape[0] - (n_res_atomize + flank + 1), (1,)
        )
        ...
        for i_end in range(i_start + 1, i_start + n_res_atomize):
            if i_end not in can_atomize_idx:
                ...
                break
        res_idxs_to_atomize = torch.arange(
            start=int(i_start), end=int(i_start + n_res_atomize)
        )
    ...
    (
        ...
        res_idxs_to_atomize, msa_prot, xyz_prot, mask_prot, bond_feats, same_chain
    )
    ...
    if torch.rand(1) < params["P_ATOMIZE_TEMPLATE"]:
        xyz_t_sm, f1d_t_sm, mask_t_sm = spoof_template(
            xyz[0, protein_L:],
            seq_atomize_all,
            mask[0, protein_L:],
            atom_template_motif_idxs,
        )
    else:
        tplt_sm = {"ids": []}
        xyz_t_sm, f1d_t_sm, mask_t_sm, _ = TemplFeaturize(
...
def loader_atomize_complex(
    ...
    xyz_t_prot = torch.stack(
        [
            center_and_realign_missing(
                xyz_t_prot[i], mask_t_prot[i], same_chain=same_chain
            )
            for i in range(ntempl)
        ]
    )
    ...
    if sum(L_s) > crop_len:
        ...
        crop_idx = get_spatial_crop(
            xyz, mask, torch.arange(sum(L_s)), L_s, params_temp, pdb_pair
        )
    else:
    ...
    if flank + 1 >= can_atomize_idx.shape[0] - (n_res_atomize + flank + 1):
        ...
        chirals = torch.Tensor()
        L_s = [
            torch.sum(crop_idx < L_s[0]).numpy(),
            torch.sum(crop_idx >= L_s[0]).numpy(),
        ]
        seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
            msa_prot, ins_prot, params, L_s=L_s
        )
        ch_label = torch.zeros(seq[0].shape)
        ch_label[L_s[0] :] = 1
...
def loader_sm(item, params, pick_top=True):
    ...
    # Load small molecule
    fname = (
        params["CSD_DIR"] + "/torch/" + item["label"][:2] + "/" + item["label"] + ".pt"
    )
    ...
    if (
        xyz_sm.shape[0] > params["MAXNSYMM"]
    ):  # clip no. of symmetry variants to save GPU memory
    ...
    xyz_t, f1d_t, mask_t, _ = TemplFeaturize(
        {"ids": []}, sm_L, params, offset=0, npick=0, pick_top=pick_top
    )
    ntempl = xyz_t.shape[0]
    xyz_t = torch.stack(
        [
...
def sample_item(df, ID, rng=None):
    ...
    return copy.deepcopy(
        item
    )  # prevents dataframe from being modified by downstream changes
...
class DatasetComplex(data.Dataset):
    def __init__(
        self, IDs, loader, data_df, params, pick_top=True, negative=False, seed=None
    ):
        self.IDs = IDs
        self.data_df = data_df
        self.loader = loader
    ...
    def __getitem__(self, index):
        ...
        item = sample_item(self.data_df, ID, self.rng)
        out = self.loader(
            item, self.params, pick_top=self.pick_top, negative=self.negative
        )
        return out
...
class DatasetSMComplex(data.Dataset):
    def __init__(
        ...
        data_df,
        params,
        init_protein_tmpl=False,
        init_ligand_tmpl=False,
...
class DatasetSMComplexAssembly(data.Dataset):
    def __init__(
        ...
        seed=None,
        select_farthest_residues: bool = False,
        load_ligand_from_column: Optional[str] = None,
        ligand_column_string_format: str = "sdf",
    ...
    def __getitem__(self, index):
        ...
        if self.load_ligand_from_column is not None:
            ...
            if (
                self.ligand_dictionary is not None
                and chosen_ligand in self.ligand_dictionary
            ):
...
class DistilledDataset(data.Dataset):
    def __init__(
        ...
        for index, (key, dataset_name) in enumerate(
            zip(self.index_dict.keys(), self.correct_dataset_ordering)
        ):
            error_message = f"Expected dataset {dataset_name} at index {index}, but you provided dataset {key}. "
            error_message += (
                "See DistilledDataset for the correct dataset names and ordering."
            )
        ...
        if True:
            ...
            if index >= offset and index < offset + len(self.index_dict["pdb"]):
                ...
                out = self.loader_dict["pdb"](
                    item,
                    self.params,
                    chid2hash=self.chid2hash,
                    homo=self.homo,
                    p_homo_cut=self.p_homo_cut,
                    p_short_crop=self.p_short_crop,
                    p_dslf_crop=self.p_dslf_crop,
            ...
            if index >= offset and index < offset + len(
                self.index_dict["neg_na_compl"]
            ):
                task = "neg_na_compl"
                ID = self.ID_dict["neg_na_compl"][index - offset]
                item = sample_item(self.dataset_dict["neg_na_compl"], ID)
                out = self.loader_dict["neg_na_compl"](
                    item, self.params, negative=True, native_NA_frac=self.native_NA_frac
                )
            ...
            if index >= offset and index < offset + len(self.index_dict["sm_compl"]):
                ...
                out = self.loader_dict["sm_compl"](
                    item,
                    self.params,
                    self.chid2hash,
                    self.chid2taxid,
                    self.chid2smpartners,
                    task="sm_compl",
            ...
            if index >= offset and index < offset + len(self.index_dict["metal_compl"]):
                ...
                out = self.loader_dict["metal_compl"](
                    item,
                    self.params,
                    self.chid2hash,
                    self.chid2taxid,
            ...
            if index >= offset and index < offset + len(
                self.index_dict["sm_compl_covale"]
            ):
                ...
                out = self.loader_dict["sm_compl_covale"](
                    item, self.params, self.chid2hash, self.chid2taxid, task=task
                )
            offset += len(self.index_dict["sm_compl_covale"])

            if index >= offset and index < offset + len(
                self.index_dict["sm_compl_asmb"]
            ):
                task = "sm_compl_asmb"
                ID = self.ID_dict["sm_compl_asmb"][index - offset]
                item = sample_item_sm_compl(self.dataset_dict["sm_compl_asmb"], ID)
                out = self.loader_dict["sm_compl_asmb"](
                    item, self.params, self.chid2hash, self.chid2taxid, task=task
                )
            ...
            if index >= offset and index < offset + len(self.index_dict["atomize_pdb"]):
                ...
                n_res_atomize = np.random.randint(
                    self.params["NRES_ATOMIZE_MIN"], self.params["NRES_ATOMIZE_MAX"] + 1
                )
                out = self.loader_dict["atomize_pdb"](
                    item,
                    self.params,
                    self.homo,
                    n_res_atomize,
                    self.params["ATOMIZE_FLANK"],
                    unclamp=(p_unclamp > self.unclamp_cut),
                )
            offset += len(self.index_dict["atomize_pdb"])

            if index >= offset and index < offset + len(
                self.index_dict["atomize_complex"]
            ):
                ...
                out = self.loader_dict["atomize_complex"](
                    item,
                    self.params,
                    self.homo,
                    n_res_atomize,
