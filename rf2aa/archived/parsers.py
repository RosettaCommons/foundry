...
def get_dislf(seq, xyz, mask):
    L = seq.shape[0]
    resolved_cys_mask = (
        ((seq == ChemData().aa2num["CYS"]) * mask[:, 5]).nonzero().squeeze(-1)
    )  # cys[5]=='sg'
...
def read_template_pdb(L, pdb_fn, target_chain=None):
    ...
    with open(pdb_fn) as fp:
        for line in fp:
            ...
            aa = line[17:20]
            seq_full.append(
                ChemData().aa2num[aa] if aa in ChemData().aa2num.keys() else 20
            )
...
def read_multichain_pdb(pdb_fn, tmpl_chain=None, tmpl_conf=0.1):
    ...
    conf = torch.where(
        mask_t[..., :3].all(dim=-1)[..., None],
        torch.full((1, L, 1), tmpl_conf),
        torch.zeros(L, 1),
    ).float()

    seq_1hot = torch.nn.functional.one_hot(
        seq, num_classes=ChemData().NAATOKENS - 1
    ).float()
    t1d = torch.cat((seq_1hot, conf), -1)
...
def parse_fasta(filename, maxseq=10000, rmsa_alphabet=False):
    ...
    if rmsa_alphabet:
        alphabet = np.array(list("00000000000000000000-000000ACGTN"), dtype="|S1").view(
            np.uint8
        )
    else:
        alphabet = np.array(list("ARNDCQEGHILKMFPSTWYV-0acgtxbdhuy"), dtype="|S1").view(
            np.uint8
        )
...
def parse_multichain_fasta(
    filename, maxseq=10000, rna_alphabet=False, dna_alphabet=False
):
    msa = []
    ins = []
...
def parse_pdb_lines_w_seq(lines, lddt_mask=False):
    ...
    res = [
        (l[21:22].strip(), l[22:26], l[17:20], l[60:66].strip())
        for l in lines
        if l[:4] == "ATOM" and l[12:16].strip() in ["CA", "P"]
    ]  # (chain letter, res num, aa)
    pdb_idx_s = [(r[0], int(r[1])) for r in res]
    idx_s = [int(r[1]) for r in res]
    plddt = [float(r[3]) for r in res]
    seq = [
        ChemData().aa2num[r[2]] if r[2] in ChemData().aa2num.keys() else 20 for r in res
    ]
...
def parse_templates_raw(ffdb, hhr_fn, atab_fn, max_templ=20):
    ...
    return (
        ...
        torch.from_numpy(qmap),
        torch.from_numpy(f0d),
        torch.from_numpy(f1d),
...
def read_templates(qlen, ffdb, hhr_fn, atab_fn, n_templ=10):
    xyz_t, mask_t, qmap, t1d, seq, ids = parse_templates_raw(
        ffdb, hhr_fn, atab_fn, max_templ=max(n_templ, 20)
    )
    ...
    if npick < 1:  # no templates
        ...
        t1d = torch.nn.functional.one_hot(
            torch.full((1, qlen), 20).long(), num_classes=21
        ).float()  # all gaps
...
def parse_mol(
    ...
    if generate_conformer:
        ...
        if did_setup:
            ...
            raise ValueError(
                f"Failed to generate 3D coordinates for molecule {filename}."
            )
    if remove_H:
        ...
        while i < obmol.NumAtoms() + 1:
            if obmol.GetAtom(i).GetAtomicNum() == 1:
                ...
                i += 1
    atomtypes = [
        ChemData().atomnum2atomtype.get(obmol.GetAtom(i).GetAtomicNum(), "ATM")
    ...
    atom_coords = torch.tensor(
        [
            [obmol.GetAtom(i).x(), obmol.GetAtom(i).y(), obmol.GetAtom(i).z()]
            for i in range(1, obmol.NumAtoms() + 1)
        ]
    ).unsqueeze(0)  # (1, natoms, 3)
