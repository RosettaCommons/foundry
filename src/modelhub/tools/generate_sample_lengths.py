...
from modelhub.chemical import load_pdb_ideal_sdf_strings
...
def parse_arguments():
    ...
    parser.add_argument(
        "--crop-size", type=int, help="Crop size for the data loader", default=256
    )
    parser.add_argument(
        "--msa-limit", type=int, help="MSA limit for the data loader", default=None
    )
    parser.add_argument(
        "--output",
        type=str,
        help="Output file path for the lengths",
        default="sample_lengths.pt",
    )
    parser.add_argument(
        "--num-samples", type=int, help="Number of samples for the loop", default=None
    )
    parser.add_argument(
...
def main(crop_size, msa_limit, output_file, num_samples, num_workers):
    ...
    assert len(set(fixed_length_datasets) & set(variable_length_datasets)) == 0, (
        "Monomer and variable datasets should not overlap"
    )
    ...
    for key, index_list in train_set.index_dict.items():
        ...
        if key in variable_length_datasets:
            ...
            if key in ["pdb", "fb"]:
                train_set.dataset_dict[key]["LENGTH"] = train_set.dataset_dict[key][
                    "SEQUENCE"
                ].apply(len)
            else:
                train_set.dataset_dict[key]["LENGTH"] = train_set.dataset_dict[key][
            ...
            fixed_lengths = train_set.dataset_dict[key].drop_duplicates(
                subset=["CLUSTER"]
            )

            # Create a dictionary for mapping cluster ID to length
            cluster_length_map = dict(
                zip(fixed_lengths["CLUSTER"], fixed_lengths["LENGTH"])
            )
            ...
            lengths[adjusted_index_list] = torch.tensor(
                fixed_lengths_processed, dtype=torch.float
            )
    ...
    for batch_idx, batch in tqdm(
        enumerate(train_loader), total=num_samples, desc="Processing batches"
    ):
        try:
...
if __name__ == "__main__":
    ...
    seed_everything()
    main(
        args.crop_size, args.msa_limit, args.output, args.num_samples, args.num_workers
    )
