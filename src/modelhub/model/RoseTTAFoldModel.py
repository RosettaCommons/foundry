...
import modelhub.util
from modelhub.chemical import ChemicalData as ChemData
from modelhub.model.layers.AuxiliaryPredictor import (
    BinderNetwork,
    DistanceNetwork,
    LDDTNetwork,
    MaskedTokenNetwork,
...
from modelhub.model.Track_module import IterativeSimulator
from modelhub.tensor_util import assert_equal, assert_shape
...
class LegacyRoseTTAFoldModule(nn.Module):
    ...
    def forward(
        ...
        # is_sm = modelhub.util.is_atom(seq[0])  # (L) (prot + non-protein)
        is_prot = modelhub.util.is_protein(seq[0])  # (L) (protein)()
        ...
        if self.verbose_checks:
            ...
            pseq_0 = pseq_0[0]
            ic(
                f"motif    sequence: {modelhub.util.seq2chars(torch.argmax(pseq_0[is_motif], dim=-1).tolist())}"  # modelhub.chemical.seq2chars is not callable
            )
            ic(
                f"diffused sequence: {modelhub.util.seq2chars(torch.argmax(pseq_0[~is_motif], dim=-1).tolist())}"
            )
