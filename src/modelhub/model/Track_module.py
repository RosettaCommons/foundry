...
from modelhub.kinematics import Qs2Rs, normQ
from modelhub.loss.loss import (
    calc_chiral_grads,
    calc_lj_grads,
)
from modelhub.model.layers.Attention_module import *
from modelhub.model.layers.SE3_network import SE3TransformerWrapper
from modelhub.util import is_atom, xyz_frame_from_rotation_mask
...
class PositionalEncoding2D(nn.Module):
    ...
    def __init__(
        ...
        self.nbin_res = (
            abs(minpos) + maxpos + 2
        )  # include 0 and "unknown" value (maxpos+1)
        self.nbin_atom = (
            maxpos_atom + 2
        )  # include 0 and "unknown" token (maxpos_sm + 1)
    ...
    def forward(self, seq, idx, bond_feats, dist_matrix, same_chain=None, cyclize=None):
        ...
        res_dist, atom_dist = get_res_atom_dist(
            idx,
            bond_feats,
            dist_matrix,
            sm_mask,
            minpos_res=self.minpos,
            maxpos_res=self.maxpos,
            maxpos_atom=self.maxpos_atom,
            cyclize=cyclize,
        )
        ...
        if (
            self.use_same_chain
            and self.enable_same_chain == False
            and same_chain is not None
        ):
...
class MSAPairStr2MSA(nn.Module):
    def __init__(
        ...
        if use_global_attn:
            ...
            self.col_attn = attention_module(
...
def find_symmsub(Ltot, Lasu, k, pseudo_cycle=False):
    ...
    if pseudo_cycle:
        ...
        assert torch.sum(symmsub == -1) == 0, (
            "Current symmsub not compatible with pseudocycle, increase symmsub_k to nrepeat-1"
        )
...
def max_block_activations(pair, symmsub):
        ...
        for j in range(Osub):
            ...
            if sij >= 0:
                if not stacks.get(int(sij), False):
                    stacks[int(sij)] = []
                stacks[int(sij)].append(
                    pair[0, i * Leff : (i + 1) * Leff, j * Leff : (j + 1) * Leff]
                )
    ...
    for i in range(Osub):
        for j in range(Osub):
            sij = symmsub[i, j]
            if sij >= 0:
                pair[0, i * Leff : (i + 1) * Leff, j * Leff : (j + 1) * Leff] = stacks[
                    int(sij)
                ]  # pairsymm[sij]/Nsymm[sij]
    return pair
...
def mean_block_activations(pair, symmsub):
    ...
    # applies block averaging to the pair representation based on symmsub
    pairsymm = torch.zeros(
        [Osub, Leff, Leff, pair.shape[-1]], device=pair.device, dtype=pair.dtype
    )
    Nsymm = torch.zeros([Osub], device=pair.device, dtype=torch.int)
    ...
    for i in range(Osub):
        for j in range(Osub):
            ...
            if sij >= 0:
                pairsymm[sij] += pair[
                    0, i * Leff : (i + 1) * Leff, j * Leff : (j + 1) * Leff
                ]
                Nsymm[sij] += 1
...
def apply_pair_symmetry(pair, symmsub, method="mean", main_block=None):
    ...
    if method == "mean":
        ...
        assert main_block is not None, "cant have None main block here"
...
class PairStr2Pair(nn.Module):
    ...
    def subblock(self, OP, pair, rbf_feat, crop):
        ...
        pairnew = torch.zeros(
            (N, L * L, pair.shape[-1]), device=pair.device, dtype=pair.dtype
        )
        ...
        for i in range(ncrops):
            # reindex sub-blocks
            offsetC = torch.clamp(
                (1 + idx[1, i : (i + 1)]) * (crop // 2) - L, min=0
            )  # account for going past L
            ...
            pairIdx[:, : (crop // 2)] = (
                torch.arange(crop // 2, dtype=torch.long, device=pair.device)
                + idx[0, i : (i + 1), None] * (crop // 2)
                - offsetN[:, None]
            )
            pairIdx[:, (crop // 2) :] = (
                torch.arange(crop // 2, dtype=torch.long, device=pair.device)
                + idx[1, i : (i + 1), None] * (crop // 2)
                - offsetC[:, None]
            )
            ...
            iUL = (iL * L + iU).flatten()
            pairnew.index_add_(
                1, iUL, paircrop.reshape(N, iUL.shape[0], pair.shape[-1])
            )
            countnew.index_add_(
                1,
                iUL,
                torch.ones((N, iUL.shape[0]), device=pair.device, dtype=torch.int),
            )
    ...
    def forward(
        ...
        if crop > 0 and crop <= L:
            pair = self.subblock(
                lambda x, y: self.drop_row(self.tri_mul_out(x)), pair, rbf_feat, crop
            )

            pair = self.subblock(
                lambda x, y: self.drop_row(self.tri_mul_in(x)), pair, rbf_feat, crop
            )

            pair = self.subblock(
                lambda x, y: self.drop_row(self.row_attn(x, y)), pair, rbf_feat, crop
            )

            pair = self.subblock(
                lambda x, y: self.drop_col(self.col_attn(x, y)), pair, rbf_feat, crop
            )
        ...
        if self.symmetrize_repeats:
            ...
            Lprot = is_prot.sum()
            symmsub = find_symmsub(
                Lprot, self.repeat_length, self.symmsub_k, self.pseudo_cycle
            )
        else:
            symmsub = None
        if (symmsub is not None) and (pair_symm_ok) and (pre_symm_permutation is None):
            ...
            symm_out = apply_pair_symmetry(
                pair_to_symm, symmsub, self.sym_method, self.main_block
            )
            ...
            (symmsub is not None)
            and (pair_symm_ok)
            and (pre_symm_permutation is not None)
        ):
            # assumes the small molecules will be part of the symmetrization, and permutation will take care of it
            assert torch.is_tensor(pre_symm_permutation), (
                "pre_symm_permutation must be a tensor"
            )
...
class Str2Str(nn.Module):
    def __init__(
        ...
        self.ff_edge = FeedForwardLayer(
            SE3_param["num_edge_features"], 2, p_drop=p_drop
        )
    ...
    def forward(
        ...
        neighbor = get_seqsep_protein_sm(
            idx, bond_feats, dist_matrix, rotation_mask, cyclize=cyclic_reses
        )
        ...
        xyz = (
            torch.einsum("blij,blaj->blai", Rout, v)
            + xyz[:, :, 1:2, :]
            + T[:, :, None, :]
        )
...
class Allatom2Allatom(nn.Module):
    def __init__(self, SE3_param):
        super(Allatom2Allatom, self).__init__()

        self.se3 = SE3TransformerWrapper(**SE3_param)

    @torch.cuda.amp.autocast(enabled=False)
    def forward(self, seq, xyz, aamask, num_bonds, state, grads, top_k=24, eps=1e-4):
...
class AllatomEmbed(nn.Module):
    def __init__(self, d_state_in=64, d_state_out=32, p_mask=0.15):
        ...
        self.compress_embed = nn.Linear(
            d_state_in + 29, d_state_out
        )  # 29->5 if using element
    ...
    def forward(self, state, seq, eltmap):
        ...
        state = self.compress_embed(torch.cat((state, elements), dim=-1))
        state = self.norm_state(state)
...
class ResidueEmbed(nn.Module):
    def __init__(self, d_state_in=16, d_state_out=64):
...
def update_symm_Rs(
    ...
    def dist_error_comp(R0, T0, xyz, tscale):
        ...
        Tcom = xyz[:, :Lasu].mean(
            dim=1, keepdim=True
        )  # LT center of mass for first ASU
        Tcorr = (
            torch.einsum("ij,brj->bri", R0, xyz[:, :Lasu] - Tcom)
            + Tcom
            + tscale * T0[None, None, :]
        )  # LT Rotated coordinates of first ASU by learned R0, then translated by learned T0
    ...
    if fit:
        ...
        with torch.enable_grad():
            ...
            lbfgs = torch.optim.LBFGS(
                [T0, Q0], history_size=10, max_iter=4, line_search_fn="strong_wolfe"
            )
            ...
            T0 = T0.detach()
            xyz = (
                torch.einsum("ij,braj->brai", Q2R(Q0), xyz[:, :Lasu] - Tcom)
                + Tcom
                + tscale * T0[None, None, :]
            )
...
def update_symm_subs(
    ...
    pair = pair.view(1, -1, pair.shape[-1])[:, idx.flatten(), :].view(
        1, Osub * L, Osub * L, pair.shape[-1]
    )
...
class IterBlock(nn.Module):
    ...
    def forward(
        ...
        cas = xyz[:, :, 1].contiguous()
        rbf_feat = rbf(torch.cdist(cas, cas)) + self.pos(
            seq_unmasked, idx, bond_feats, dist_matrix, same_chain
        )
        if self.detach_xyz:
            xyz = xyz.detach()
        if use_checkpoint:
            msa = checkpoint.checkpoint(
                create_custom_forward(self.msa2msa),
                msa,
                pair,
                rbf_feat,
                state,
            ...
            pair = checkpoint.checkpoint(
                create_custom_forward(self.msa2pair), msa, pair, use_reentrant=True
            )
            ...
            xyz, state, alpha, quat = checkpoint.checkpoint(
                create_custom_forward(
                    self.str2str,
                    top_k=top_k,
        ...
        if symmsub is not None and symmsub.shape[0] > 1 and pair_symm_ok:
            xyz, pair, symmsub = update_symm_subs(
                xyz,
                pair,
                symmids,
                symmsub,
...
class IterativeSimulator(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "lj_correction_parameters", lj_correction_parameters, persistent=False
        )
    ...
    def forward(
        ...
        for i_m in range(self.n_extra_block):
            ...
            msa_full, pair, xyz, state, alpha, symmsub, quat = self.extra_block[i_m](
                msa_full,
                pair,
                xyz,
                state,
        ...
        for i_m in range(self.n_main_block):
            ...
            if self.use_chiral_l1:
                (dchiraldxyz,) = calc_chiral_grads(xyz.detach(), chirals)
                extra_l1 = dchiraldxyz[0].detach()
            msa, pair, xyz, state, alpha, symmsub, quat = self.main_block[i_m](
                msa,
                pair,
                xyz,
                state,
        ...
        _, xyzallatom = self.xyzconverter.compute_all_atom(
            seq_unmasked, xyz, alpha
        )  # think about detach here...
            ...
            with ExitStack() as stack:
                ...
                if self.use_lj_l1:
                    dljdxyz, dljdalpha = calc_lj_grads(
                        ...
                        self.num_bonds,
                        lj_lin=self.lj_lin,
                        norm_by_atoms_twice=False,
                    )  # LT norm_by_atoms_twice should be True for inference and False for rf2aa
                    extra_l0 = dljdalpha.reshape(
                        1, -1, 2 * ChemData().NTOTALDOFS
                    ).detach()
                ...
                xyz, state, alpha, quat = self.str_refiner(
                    ...
                    extra_l0,
                    extra_l1.float(),
                    top_k=self.refiner_topk,
                    use_atom_frames=use_atom_frames,
                )

                if symmsub is not None and symmsub.shape[0] > 1 and pair_symm_ok:
                    xyz = update_symm_Rs(
                        xyz, Lasu, symmsub, symmRs, recenter_particle=recenter_particle
                    )
