...
from modelhub.util_module import Dropout, init_lecun_normal
...
class AF3_full_block(nn.Module):
    ...
    def __init__(self, global_config, block_params, **kwargs):
        ...
        d_msa, d_pair = (global_config.d_msa_full, global_config.d_pair)
...
class AF3_block(nn.Module):
    ...
    def __init__(self, global_config, block_params, **kwargs):
        ...
        d_singleseq, d_pair = (global_config.d_msa, global_config.d_pair)
        ...
        self.msa_transition = FeedForwardLayer(
            d_singleseq, 4, p_drop=block_params.msa_transition_drop
        )
    ...
    def forward(self, latent_feats, use_checkpoint, use_amp):
        ...
        if use_checkpoint:
            ...
            pair = checkpoint.checkpoint(
                self._2d_update, singleseq, pair, use_reentrant=True
            )
            singleseq = checkpoint.checkpoint(
                self._1d_update, singleseq, pair, use_reentrant=True
            )
        else:
            # 2d then 1d update
