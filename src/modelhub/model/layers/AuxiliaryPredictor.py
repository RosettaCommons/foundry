...
class DistanceNetwork(nn.Module):
    def __init__(self, n_feat, p_drop=0.0):
        ...
        self.proj_symm = nn.Linear(
            n_feat, 61 + 37
        )  # must match bin counts defined in kinematics.py
...
class BinderNetwork(nn.Module):
    ...
    def forward(self, pae, same_chain):
        ...
        logits_inter = torch.mean(
            logits[same_chain == 0], dim=0
        ).nan_to_num()  # all zeros if single chain
