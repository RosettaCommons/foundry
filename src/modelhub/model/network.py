...
from modelhub.model.embedding_blocks import embedding_factory
from modelhub.model.layers.AuxiliaryPredictor import aux_predictor_factory
from modelhub.model.refinement_blocks import refinement_factory
from modelhub.model.simulator_blocks import block_factory
...
class RosettaFold(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.embedding = embedding_factory[embedding_type](
            model_params["global_params"],
            model_params.embedding[embedding_type]["params"],
        )
        ...
        for block in model_params.blocks.keys():
            if block not in block_factory:
                raise ValueError(
                    f"User specified {block} type, but this block is not registered in modelhub.Trunk_blocks."
                )
            blocks_to_add = [
                block_factory[block](
        ...
        for aux_task in model_params.auxiliary_predictors.keys():
            aux_tasks.update(
                {
                    aux_task: aux_predictor_factory[aux_task](
