...
from modelhub.loss.loss import calc_chiral_grads
from modelhub.model.generative_refinement import GenerativeRefinement
from modelhub.model.layers.SE3_network import (
    FullyConnectedSE3,
)
from modelhub.model.Track_module import Str2Str
from modelhub.util_module import init_lecun_normal, make_full_graph, make_topk_graph
...
class LocalRefinementSE3(FullyConnectedSE3):
    def __init__(self, global_config, block_params):
        ...
        super(LocalRefinementSE3, self).__init__(
            d_msa,
            d_pair,
            d_rbf,
            num_layers,
    ...
    def construct_graph(self, xyz, edge):
        ...
        if self.top_k > 0:
            G, edge_feats = make_topk_graph(
                xyz[:, :, 1, :], edge, idx, top_k=self.top_k
            )
        else:
            G, edge_feats = make_full_graph(
                xyz[:, :, 1, :], edge, idx
            )  # top_k=1 -> fully connected
...
class RecurrentLocalRefinement_w_Adaptor(nn.Module):
    def __init__(self, global_config, block_params):
        ...
        self.proj_state_in = nn.Linear(
            block_params.adaptor_features, block_params.l0_in_features
        )
        self.proj_state_out = nn.Linear(
            block_params.l0_in_features, block_params.adaptor_features
        )
