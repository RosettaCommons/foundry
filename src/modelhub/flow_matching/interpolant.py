...
from modelhub.flow_matching import so3_utils  # , all_atom
...
def _masked_categorical(num_batch, num_res, device):
    return torch.ones(num_batch, num_res, device=device) * ChemData().MASK_INDEX
...
class Interpolant:
    def __init__(self, cfg):
        ...
        self.num_tokens = (
            self._aatypes_cfg.num_tokens + 1
            if self._aatypes_cfg.interpolant_type == "masking"
            else self._aatypes_cfg.num_tokens
        )
    ...
    def _corrupt_trans(self, trans_1, t, res_mask, diffuse_mask):
        ...
        if self._trans_cfg.train_schedule == "linear":
            ...
            trans_t = (
                torch.sqrt(alpha_t[..., None]) * trans_1
                + torch.sqrt(1 - alpha_t[..., None]) * trans_0
            )
        else:
    ...
    def _batch_ot(self, trans_0, trans_1, res_mask):
        ...
        cost_matrix = torch.sum(
            torch.linalg.norm(
                batch_mask[..., None]
                * (aligned_nm_0.nan_to_num() - aligned_nm_1.nan_to_num()),
                dim=-1,
            ),
            dim=-1,
    ...
    def _corrupt_aatypes(self, aatypes_1, t, res_mask, diffuse_mask):
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            if self._aatypes_cfg.schedule == "linear":
                ...
                corruption_mask = u < (
                    1 - (1 / (math.exp(coeff) - 1)) * (torch.exp(coeff * t) - 1)
                )
            else:
                raise ValueError(
                    f"Unknown aatypes schedule {self._aatypes_cfg.schedule}"
                )
            ...
            raise ValueError(
                f"Unknown aatypes interpolant type {self._aatypes_cfg.interpolant_type}"
            )
    ...
    def corrupt_batch(self, batch):
        ...
        if self._cfg.separate_t:
            if self._cfg.hierarchical_t:
                max_t = torch.rand(num_batch, device=self._device) * (
                    1 - self._cfg.min_t
                )
                so3_t = self._cfg.min_t + torch.rand(num_batch, device=self._device) * (
                    max_t - self._cfg.min_t
                )
                r3_t = self._cfg.min_t + torch.rand(num_batch, device=self._device) * (
                    max_t - self._cfg.min_t
                )
                cat_t = self._cfg.min_t + torch.rand(num_batch, device=self._device) * (
                    max_t - self._cfg.min_t
                )
                so3_t = so3_t[:, None]
                r3_t = r3_t[:, None]
            ...
            inverse_fold_mask = (
                u
                < self._cfg.codesign_inverse_fold_prop
                + self._cfg.codesign_forward_fold_prop
            ).float() * (u >= self._cfg.codesign_forward_fold_prop).float()
        ...
        if self._aatypes_cfg.corrupt:
            aatypes_t = self._corrupt_aatypes(
                aatypes_1, cat_t, res_mask, diffuse_mask_seq
            )
        else:
            aatypes_t = aatypes_1
        noisy_batch["aatypes_t"] = aatypes_t
        noisy_batch["trans_sc"] = torch.zeros_like(trans_1)
        noisy_batch["aatypes_sc"] = torch.zeros_like(aatypes_1)[..., None].repeat(
            1, 1, self.num_tokens
        )
        return noisy_batch
    ...
    def _aatypes_euler_step(self, d_t, t, logits_1, aatypes_t):
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            pt_x1_probs = F.softmax(
                logits_1 / self._aatypes_cfg.temp, dim=-1
            )  # (B, D, S)

            aatypes_t_is_mask = (
                (aatypes_t == du.MASK_TOKEN_INDEX).view(batch_size, num_res, 1).float()
            )
            if self._aatypes_cfg.schedule == "linear":
                step_probs = (
                    d_t * pt_x1_probs * ((1 + self._aatypes_cfg.noise * t) / (1 - t))
                )  # (B, D, S)
            elif self._aatypes_cfg.schedule == "exp":
                coeff = self._aatypes_cfg.schedule_exp_rate
                alpha_t = (1 / (math.exp(coeff) - 1)) * (torch.exp(coeff * t) - 1)
                dt_alpha_t = (1 / (math.exp(coeff) - 1)) * (
                    coeff * torch.exp(coeff * t)
                )
                step_probs = (
                    d_t
                    * pt_x1_probs
                    * ((dt_alpha_t + self._aatypes_cfg.noise * alpha_t) / (1 - alpha_t))
                )  # (B, D, S)
            step_probs += (
                d_t
                * (1 - aatypes_t_is_mask)
                * mask_one_hot.view(1, 1, -1)
                * self._aatypes_cfg.noise
            )
    ...
    def _aatypes_euler_step_purity(self, d_t, t, logits_1, aatypes_t):
        ...
        pt_x1_probs = F.softmax(
            logits_1_wo_mask / self._aatypes_cfg.temp, dim=-1
        )  # (B, D, S-1)
        # assert self._aatypes_cfg.schedule == 'linear'
        ...
        if self._aatypes_cfg.schedule == "linear":
            unmask_probs = (
                d_t * ((1 + self._aatypes_cfg.noise * t) / (1 - t)).to(device)
            ).clamp(max=1)  # scalar
        elif self._aatypes_cfg.schedule == "exp":
            assert self._aatypes_cfg.noise == 0.0, (
                "If you want noise, double check the calcs for exp schedule"
            )
            coeff = self._aatypes_cfg.schedule_exp_rate
            alpha_t = (1 / (math.exp(coeff) - 1)) * (torch.exp(coeff * t) - 1)
            dt_alpha_t = (1 / (math.exp(coeff) - 1)) * (coeff * torch.exp(coeff * t))
            unmask_probs = (
                (d_t * (dt_alpha_t / (1 - alpha_t))).to(device).clamp(max=1)
            )  # scalar
        elif self._aatypes_cfg.schedule == "never_decode":
            unmask_probs = torch.zeros_like(t).to(device)
        else:
            raise Exception(
                f"unknown interpolant.aatypes.schedule {self._aatypes_cfg.schedul}"
            )

        number_to_unmask = torch.binomial(
            count=torch.count_nonzero(aatypes_t == du.MASK_TOKEN_INDEX, dim=-1).float(),
            prob=unmask_probs,
        )
    ...
    def sample(
        ...
        if diffuse_mask_seq is None:
            raise Exception("diffuse_mask_seq is None.")
        ...
        if (
            diffuse_mask is not None
            and trans_1 is not None
            and rotmats_1 is not None
            and aatypes_1 is not None
        ):
        ...
        if motif_scaffolding and not self._cfg.twisting.use:  # amortisation
            ...
            aatypes_0 = _aatypes_diffuse_mask(
                aatypes_0, aatypes_1, diffuse_mask_seq.long()
            )
            batch["trans_sc"] = _trans_diffuse_mask(trans_sc, trans_1, diffuse_mask)
            logits_1 = torch.nn.functional.one_hot(
                aatypes_1.long(), num_classes=self.num_tokens
            ).float()
        ...
        prot_traj = [
            (frames_to_atom37(trans_t_1, rotmats_t_1), aatypes_0.detach().cpu().long())
        ]
        clean_traj = []
        for t_2 in ts[1:]:
            # Run model.
            ...
            if not self._cfg.twisting.use:
                ...
                aatypes_t_2 = _aatypes_diffuse_mask(
                    aatypes_t_2, aatypes_1, diffuse_mask
                )
            trans_t_1, rotmats_t_1, aatypes_t_1 = trans_t_2, rotmats_t_2, aatypes_t_2
            prot_traj.append(
                (
                    frames_to_atom37(trans_t_2, rotmats_t_2),
                    aatypes_t_2.cpu().detach().long(),
                )
            )
