...
import modelhub.flow_matching.data_utils as du
...
from modelhub.flow_matching import data_transforms
from modelhub.flow_matching.interpolant import _centered_gaussian, _uniform_so3
from modelhub.training.recycling import (
    recycle_step_gen,
    recycle_step_packed,
    unpack_outputs,
)
from modelhub.util import rigid_from_3_points, writepdb_file


class Sampler:
    def __init__(
        self, model, num_timesteps, min_t, interpolant, xyz_converter, is_training
    ) -> None:
    ...
    def sample(self, inputs: Tuple[str, Any], use_amp=False) -> Dict[str, Any]:
        ...
        for t_2 in ts[1:]:
            ...
            updated_features = self._construct_xt_features(
                rotmats_t_1, trans_t_1, network_input
            )
            network_input.update(updated_features)
            # run model
            output_i = recycle_step_packed(
                self.model, network_input, 1, use_amp=use_amp, nograds=True
            )
            xyz = output_i[5][-1]
            ...
            # take euler step
            rotmats_t_2, trans_t_2 = self._take_step(
                pred_rotmats, pred_trans, rotmats_t_1, trans_t_1, d_t, t_1
            )
    ...
    def _get_network_input(self, inputs):
        out = prepare_input_fm(
            inputs, self.interpolant, self.xyz_converter, device=self.device
        )
        network_input = out[2]
    ...
    def _setup_prior(self, seq_unmasked):
        ...
        trans_0 = _centered_gaussian(B, L, self.device) * du.NM_TO_ANG_SCALE

        rotmats_0 = _uniform_so3(B, L, self.device)
        return rotmats_0, trans_0

    def _take_step(
        self, pred_rotmats_1, pred_trans_1, rotmats_t_1, trans_t_1, d_t, t_1
    ):
    ...
    def _construct_xt_features(self, rotmats, trans, network_input):
        ...
        t1d, seq_unmasked, atom_frames = (
            network_input["t1d"],
            network_input["seq_unmasked"],
            network_input["atom_frames"],
        )
        t2d, mask_t_2d, alpha_t, alpha_prev = construct_template_feats(
            xyz[None],
            mask_t[None],
            t1d,
            seq_unmasked,
...
class AllAtomSampler(Sampler):
    ...
    def __init__(
        ...
        super().__init__(
            model, num_timesteps, min_t, interpolant, xyz_converter, is_training
        )
        raise NotImplementedError("AllAtomSampler has been retired")
        self.allatom_mask = ChemData().allatom_mask.to(self.device)

    def sample(
        self, inputs: Tuple[str, Any], n_cycle=1, use_amp=False
    ) -> Dict[str, Any]:
        # first receive inputs from dataloader
        # convert them into features
        network_input = self._get_network_input(inputs)
        ...
        output_i = recycle_step_gen(
            self.model, network_input, n_cycle, use_amp=use_amp, nograds=True
        )
        latent_feats = {
            "msa": output_i[-3],
            "pair": output_i[-2],
    ...
    def _setup_prior(self, seq_unmasked):
        ...
        xyz[is_real_atom] = (
            _centered_gaussian(B, num_atoms, self.device) * du.NM_TO_ANG_SCALE
        )
        return xyz

    def _get_network_input(self, inputs):
        out = prepare_input_fm_allatom(
            inputs, self.interpolant, self.xyz_converter, device=self.device
        )
        network_input = out[2]
        return network_input
...
class AF3Sampler:
    ...
    def sample(
        ...
        for i in range(n_cycle):
            # run the model for n_steps
            recycle_inputs["f"]["msa"] = network_input["f"]["msa_stack"][i].to(
                self.device
            )
        ...
        if self.config.af3_inference.solver == "af3":
            X_L = self.sample_diffusion(
                network_input["f"],
                post_recycle_outputs["S_inputs_I"],
                post_recycle_outputs["S_I"],
                post_recycle_outputs["Z_II"],
                noise_schedule,
            )
        ...
        if self.confidence is not None:
            ...
            outputs["confidence"] = self.confidence(
                post_recycle_outputs["S_inputs_I"],
                post_recycle_outputs["S_I"],
                post_recycle_outputs["Z_II"],
                X_L["X_L"],
                confidence_feats["rf2aa_seq"],
                confidence_feats["rep_atom_idx"],
                frame_atom_idxs=confidence_feats["frame_atom_idxs"],
            )
            # add the chain label so we can calculate ipae later
            outputs["confidence"]["chain_iid_token_lvl"] = confidence_feats[
                "chain_iid_token_lvl"
            ]
    ...
    def sample_diffusion(
        ...
        D = (
            D
            if D is not None
            else self.config.dataset_params["diffusion_batch_size_valid"]
        )
        L = f["ref_pos"].shape[0]
        ...
        for c_t_minus_1, c_t in zip(noise_schedule, noise_schedule[1:]):
            ...
            X_denoised_L = self.model.diffusion_module(
                X_noisy_L, t_hat.tile(D), f, s_inputs_I, s_trunk_I, Z_trunk_II
            )
    ...
    def sample_diffusion_simple(
        self, f, s_inputs_I, s_trunk_I, Z_trunk_II, noise_schedule, D=None
    ):
        D = (
            D
            if D is not None
        ...
        for c_t_minus_1, c_t in zip(noise_schedule, noise_schedule[1:]):
            ...
            if c_t_minus_1 != noise_schedule[0]:
                ...
                X_noisy_L = X_L
            X_denoised_L = self.model.diffusion_module(
                X_noisy_L, c_t_minus_1.tile(D), f, s_inputs_I, s_trunk_I, Z_trunk_II
            )
            X_L = X_denoised_L
    ...
    def sample_diffusion_heun(
        ...
        for c_t_minus_1, c_t in zip(noise_schedule, noise_schedule[1:]):
            ...
            X_denoised_L1 = self.model.diffusion_module(
                X_noisy_L, c_t_minus_1.tile(D), f, s_inputs_I, s_trunk_I, Z_trunk_II
            )
            ...
            if c_t != noise_schedule[-1]:
                ...
                X_denoised_L2 = self.model.diffusion_module(
                    X_L1, c_t.tile(D), f, s_inputs_I, s_trunk_I, Z_trunk_II
                )
    ...
    def _get_initial_structure(self, f, noise_schedule, D, L, device):
        X_L = noise_schedule[0] * torch.normal(
            mean=0.0, std=1.0, size=(D, L, 3), device=device
        )
        return X_L
...
class AF3PartialSampler(AF3Sampler):
    ...
    def _get_initial_structure(self, f, noise_schedule, D, L, device):
        noise = torch.normal(
            mean=0.0, std=noise_schedule[0], size=(D, L, 3), device=device
        )
    ...
    def construct_noise_schedule(self, num_timesteps, min_t, max_t):
        ...
        full_noise_schedule = super().construct_noise_schedule(
            num_timesteps, full_schedule_min_t, full_schedule_max_t
        )
