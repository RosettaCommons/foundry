...
from modelhub.kinematics import c6d_to_bins, xyz_to_c6d
from modelhub.loss.loss import (
    calc_allatom_lddt_loss,
    calc_atom_bond_loss,
    calc_BB_bond_geom,
    calc_crd_rmsd,
    calc_lddt,
    calc_lj,
    compute_general_FAPE,
    mask_unresolved_frames,
    resolve_equiv_natives,
    resolve_equiv_natives_asmb,
    resolve_symmetry,
    resolve_symmetry_predictions,
    torsionAngleLoss,
    translation_vector_field,
)
from modelhub.util import (
    Ls_from_same_chain_2d,
    get_frames,
    get_prot_sm_mask,
    is_atom,
    is_protein,
...
def get_loss_and_misc(
    ...
    if pred_allatom is None:
        _, pred_allatom = trainer.xyz_converter.compute_all_atom(
            msa[0][0][None], pred_crds[-1], alphas[-1]
        )
    ...
    if symmRs is not None:
        ...
        for li in logit_s:
            li = torch.gather(
                li,
                2,
                mapT2P[-1][None, None, :, None].repeat(1, li.shape[1], 1, li.shape[-1]),
            )
            li = torch.gather(
                li,
                3,
                mapT2P[-1][None, None, None, :].repeat(1, li.shape[1], li.shape[2], 1),
            )
            logit_s_new.append(li)
        logit_s = tuple(logit_s_new)

        logit_aa_s = logit_aa_s.view(
            1, ChemData().NAATOKENS, msa.shape[-2], msa.shape[-1]
        )
        logit_aa_s = torch.gather(
            logit_aa_s,
            3,
            mapT2P[-1][None, None, None, :].repeat(
                1, ChemData().NAATOKENS, logit_aa_s.shape[-2], 1
            ),
        )
        ...
        pred_allatom = torch.gather(
            pred_allatom,
            1,
            mapT2P[-1, None, :, None, None].repeat(1, 1, ChemData().NTOTAL, 3),
        )
        alphas = torch.gather(
            alphas,
            2,
            mapT2P[:, None, :, None, None].repeat(1, 1, 1, ChemData().NTOTALDOFS, 2),
        )

        same_chain = torch.gather(
            same_chain, 1, mapT2P[-1][None, :, None].repeat(1, 1, same_chain.shape[-1])
        )
        same_chain = torch.gather(
            same_chain, 2, mapT2P[-1][None, None, :].repeat(1, same_chain.shape[1], 1)
        )

        bond_feats = torch.gather(
            bond_feats, 1, mapT2P[-1][None, :, None].repeat(1, 1, bond_feats.shape[-1])
        )
        bond_feats = torch.gather(
            bond_feats, 2, mapT2P[-1][None, None, :].repeat(1, bond_feats.shape[1], 1)
        )
        ...
        dist_matrix = torch.gather(
            dist_matrix, 2, mapT2P[-1][None, None, :].repeat(1, dist_matrix.shape[1], 1)
        )

        pred_lddts = torch.gather(
            pred_lddts, 2, mapT2P[-1][None, None, :].repeat(1, pred_lddts.shape[-2], 1)
        )
...
def calc_loss(
    ...
    w_trans, w_inter_fape, w_lig_fape = (
        loss_param["w_trans"],
        loss_param["w_inter_fape"],
        loss_param["w_lig_fape"],
    )
    w_lddt, w_bond, w_clash = (
        loss_param["w_lddt"],
        loss_param["w_bond"],
        loss_param["w_clash"],
    )
    w_atom_bond, w_skip_bond, w_rigid = (
        loss_param["w_atom_bond"],
        loss_param["w_skip_bond"],
        loss_param["w_rigid"],
    )
    ...
    if (
        "binder" in trainer.config.model.auxiliary_predictors
        or trainer.config.experiment.trainer == "legacy"
    ):
    ...
    frame_atom_mask_2d_intra_allatom = frame_atom_mask_2d_allatom * same_chain[
        :, :, None, :, None
    ].bool().expand(-1, -1, nframes, -1, ChemData().NTOTAL)
    ...
    if task[0] in ["tf", "neg_tf"] or res_mask.sum() == 0:
        ...
        if not unclamp:
            ...
            same_chain_clamp_mask = (
                same_chain[:, :, None, :, None].bool().repeat(1, 1, nframes, 1, natoms)
            )
    ...
    true_tors, true_tors_alt, tors_mask, tors_planar = (
        trainer.xyz_converter.get_torsions(true, seq, mask_in=mask_crds)
    )
    ...
    natRs_all, _n0 = trainer.xyz_converter.compute_all_atom(
        seq, true[..., :3, :], true_tors
    )
    natRs_all_alt, _n1 = trainer.xyz_converter.compute_all_atom(
        seq, true_alt[..., :3, :], true_tors_alt
    )
    predTs = pred[-1, ...]
    predRs_all, pred_all = trainer.xyz_converter.compute_all_atom(
    ...
    l_tors = torsionAngleLoss(
        pred_tors, true_tors, true_tors_alt, tors_mask, tors_planar, eps=1e-4
    )
    ...
    clash_loss = calc_lj(
        ...
        bond_feats,
        dist_matrix,
        trainer.ljlk_parameters,
        trainer.lj_correction_parameters,
        trainer.num_bonds,
        lj_lin=lj_lin,
    )
    if w_clash > 0.0:
