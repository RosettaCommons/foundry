...
def get_seqsep_protein_sm(idx, bond_feats, dist_matrix, sm_mask, cyclize=None):
    ...
    res_dist, atom_dist = get_res_atom_dist(
        idx, bond_feats, dist_matrix, sm_mask, cyclize=cyclize
    )

    sm_mask_2d = sm_mask[None, :] * sm_mask[:, None]
    prot_mask_2d = (~sm_mask[None, :]) * (~sm_mask[:, None])
    inter_mask_2d = (~sm_mask[None, :]) * (sm_mask[:, None]) + (sm_mask[None, :]) * (
        ~sm_mask[:, None]
    )
    ...
    seqsep = (
        sm_mask_2d * atom_dist
        + prot_mask_2d * res_dist
        + inter_mask_2d * (bond_feats == 6)
    )

    return seqsep.unsqueeze(-1)


def get_res_atom_dist(
    idx,
    bond_feats,
    dist_matrix,
    sm_mask,
    ...
    res_dist_prot = torch.clamp(
        seqsep, min=minpos_res, max=maxpos_res
    )  # (L, L) intra-protein
    res_dist_sm = torch.full(
        (L, L), maxpos_res + 1, device=device
    )  # (L, L) with "unknown" res. dist. token

    # small molecule atom bond graph
    atom_dist_sm = torch.nan_to_num(dist_matrix, posinf=maxpos_atom)[
        0
    ].long()  # this comes through the dataloader so it is batched
    ...
    if i_prot.shape[0] > 0:
        closest_prot_res = i_prot[
            torch.argmin(atom_dist_sm[sm_mask, :][:, i_sm], dim=-1)
        ]
        res_dist_inter[sm_mask, :] = res_dist_prot[closest_prot_res, :]
        res_dist_inter[:, sm_mask] = res_dist_prot[:, closest_prot_res]

        closest_atom = i_sm[
            torch.argmin(torch.abs(res_dist_prot[~sm_mask, :][:, i_prot]), dim=-1)
        ]
        atom_dist_inter[~sm_mask, :] = atom_dist_sm[closest_atom, :] + 1
        atom_dist_inter[:, ~sm_mask] = atom_dist_sm[:, closest_atom] + 1

    res_dist = (
        res_dist_prot * prot_mask_2d
        + res_dist_inter * inter_mask_2d
        + res_dist_sm * sm_mask_2d
    )
    atom_dist = (
        atom_dist_prot * prot_mask_2d
        + atom_dist_inter * inter_mask_2d
        + atom_dist_sm * sm_mask_2d
    )
...
def get_relpos(idx, bond_feats, sm_mask, inter_pos=32, maxpath=32):
    ...
    bond_separation = (
        torch.full_like(bond_feats, maxpath)
    ...
    relpos = (
        prot_mask_2d * seqsep + sm_mask_2d * bond_separation + inter_mask_2d * inter_pos
    )  # (B, L, L)
...
def make_topk_graph(
    xyz, pair, idx, top_k=128, nlocal=33, topk_incl_local=True, eps=1e-4
):
    """
    Input:
        - xyz: current backbone cooordinates (B, L, 3, 3)
    ...
    D = (
        torch.cdist(xyz, xyz) + torch.eye(L, device=device).unsqueeze(0) * 9999.9
    )  # (B, L, L)
...
class XYZConverter(nn.Module):
    def __init__(self):
        ...
        self.register_buffer(
            "torsion_indices", ChemData().torsion_indices, persistent=False
        )
        self.register_buffer(
            "torsion_can_flip",
            ChemData().torsion_can_flip.to(torch.int32),
            persistent=False,
        )
        self.register_buffer(
            "ref_angles", ChemData().reference_angles, persistent=False
        )
        self.register_buffer("base_indices", ChemData().base_indices, persistent=False)
        self.register_buffer(
            "RTs_in_base_frame", ChemData().RTs_by_torsion, persistent=False
        )
        self.register_buffer(
            "xyzs_in_base_frame", ChemData().xyzs_in_base_frame, persistent=False
        )
    ...
    def get_tor_mask(self, seq, mask_in=None):
        ...
        if mask_in is not None:
            ...
            rs = (
                torch.arange(L, device=seq.device)[None, :, None, None] - (ts < 0) * 1
            )  # ts<-1 ==> prev res
            ts = torch.abs(ts)
    ...
    def get_torsions(self, xyz_in, seq, mask_in=None):
        from modelhub.chemical import ChemicalData as ChemData
        from modelhub.chemical import th_ang_v, th_dih
        ...
        xs = (
            torch.arange(L, device=xyz_in.device)[None, :, None, None] - (ts < 0) * 1
        )  # ts<-1 ==> prev res
        ys = torch.abs(ts)
        ...
        tors_planar = torch.zeros(
            (B, L, ChemData().NTOTALDOFS), dtype=torch.bool, device=xyz_in.device
        )
        tors_planar[:, :, 5] = (
            seq == ChemData().aa2num["TYR"]
        )  # TYR chi 3 should be planar
