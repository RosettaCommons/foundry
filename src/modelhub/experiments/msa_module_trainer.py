...
from modelhub.loss.af3_losses import DistogramLoss
...
from modelhub.model.layers.Embeddings import MSA_emb
from modelhub.trainer_new import ComposedTrainer
...
class MsaModuleTrainer(ComposedTrainer):
    ...
    def train_step(self, inputs, n_cycle, no_grads=False, return_outputs=False):
        ...
        network_input = {
            ...
            "X_noisy_L": torch.nan_to_num(example["ground_truth"]["coord_atom_lvl"])
            + example["noise"],
            "t": example["t"],
            "f": example["feats"],
        }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.feature_initializer = FeatureInitializer(
            c_s, c_z, c_atom, c_atompair, **feature_initializer
        )
...
class MSAModuleRFAAEmbedding(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding = MSA_emb(d_msa=c_s, d_pair=c_z, d_state=c_s_inputs, d_init=32)
        self.msa_module = MSAModule(**msa_module)
        self.dist_head = DistogramHead(c_z, **distogram_head)
