...
from modelhub.debug import pretty_describe_dict
from modelhub.flow_matching.sampler import AF3PartialSampler, AF3Sampler
from modelhub.loss.af3_losses import Loss as AF3Loss
from modelhub.loss.af3_losses import ResidueSymmetryResolution, SubunitSymmetryResolution
from modelhub.metrics.metrics_base import MetricManager
from modelhub.metrics.predicted_error import GetConfidenceIndices
from modelhub.model import AF3_structure
from modelhub.trainer_new import FlowMatchingTrainer
...
class AF3Trainer(FlowMatchingTrainer):
    def construct_model(self, device="cpu", inference=False):
        ...
        if inference is False:
            ...
            from modelhub.training.EMA import FakeDDPWrapper
    ...
    def train_step(self, inputs, n_cycle, no_grads=False, return_outputs=False):
        ...
        network_input = {
            ...
            "X_noisy_L": torch.nan_to_num(example["coord_atom_lvl_to_be_noised"])
            + example["noise"],
            "t": example["t"],
            "f": example["feats"],
        }
        ...
        loss_input = {
            "X_gt_L": example["ground_truth"]["coord_atom_lvl"][None].expand(
                self.config.dataset_params.diffusion_batch_size, -1, -1
            ),
            "crd_mask_L": example["ground_truth"]["mask_atom_lvl"][None].expand(
                self.config.dataset_params.diffusion_batch_size, -1
            ),
        ...
        loss_input = tree.map_structure(
            lambda x: x.to(gpu) if hasattr(x, "cpu") else x, loss_input
        )
        logger.debug("network_input:\n" + pretty_describe_dict(network_input))
        logger.debug("loss_input:\n" + pretty_describe_dict(loss_input))

        network_input["f"]["msa_stack"] = network_input["f"]["msa_stack"].to(
            torch.bfloat16
        )
    ...
    def valid_step(self, inputs, n_cycle, no_grads=True, return_outputs=False):
        ...
        # symmetry resolution
        loss_input = self.subunit_symm_resolve(
            outputs, loss_input, example["symmetry_resolution"]
        )
        loss_input = self.residue_symm_resolve(
            outputs, loss_input, example["automorphisms"]
        )

        metrics_dict = self.metrics(network_input, outputs, loss_input)

        if (
            self.config.eval_params.dump_cif_files
            or self.config.eval_params.dump_cif_trajectories
        ):
    ...
    def valid_epoch(self, epoch, rank, world_size):
        ...
        for dataset_name, valid_loader in self.valid_loaders.items():
            for valid_idx, inputs in enumerate(valid_loader):
                n_cycle = inputs[0]["feats"]["msa_stack"].shape[
                    0
                ]  # use size of MSA stack to set ncycles
        ...
        if rank == 0:
            self.log_validation_losses(
                epoch, all_valid_loss_dict, self.config.experiment.name
            )
    ...
    def write_pdb(self, example, outputs, dump_traj=False):
        ...
        decoded_restypes = encoding.decode(
            torch.argmax(example["feats"]["restype"], dim=-1).cpu()
        )
...
class AF3TrainerRollout(AF3Trainer):
    def construct_model(self, device="cpu", inference=False):
        ...
        model.device = device
        from modelhub.model.layers.af3_auxiliary_heads import ConfidenceHead

        confidence = ConfidenceHead(**self.config.confidence_head).to(device)
        self.confidence = confidence
        from modelhub.flow_matching.sampler import AF3Sampler
    ...
    def train_step(self, inputs, n_cycle, no_grads=False, return_outputs=False):
        ...
        network_input = tree.map_structure(
            lambda x: x.to(gpu) if hasattr(x, "cpu") else x, network_input
        )
        loss_input = tree.map_structure(
            lambda x: x.to(gpu) if hasattr(x, "cpu") else x, loss_input
        )
        ...
        loss_input = self.subunit_symm_resolve(
            output_i, loss_input, example["symmetry_resolution"]
        )
        loss_input = self.residue_symm_resolve(
            output_i, loss_input, example["automorphisms"]
        )
    ...
    def load_model(self):
        ...
        if "confidence_loss" in checkpoint_training_config["loss"]:
            logger.warning(
                "Loading weights with pretrained confidence head because confidence loss is present in the checkpoint"
            )
            super().load_model()
        else:
            logger.warning(
                "Loading weights from a model that was not trained with a confidence head. Renaming weights to be compatible with confidence head"
            )
            self.model.module.model.model.load_state_dict(
                self.checkpoint["final_state_dict"], strict=False
            )
            self.model.module.shadow.model.load_state_dict(
                self.checkpoint["model_state_dict"], strict=False
            )
            logger.info("Checkpoint loaded into model")
            logger.warning(
                "Resetting optimizer since model was not trained with confidence head"
            )
