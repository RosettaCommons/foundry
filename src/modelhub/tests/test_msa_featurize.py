...
from modelhub.data.data_loader import MSAFeaturize, cluster_sum, get_term_feats
from modelhub.data.parsers import parse_a3m
from modelhub.tensor_util import assert_equal
from modelhub.tests.test_conditions import setup_data
from modelhub.trainer_new import seed_all
...
def OldMSAFeaturize(
    ...
    # raw MSA profile
    raw_profile = torch.nn.functional.one_hot(
        msa, num_classes=ChemData().NAATOKENS
    )  # N x L x NAATOKENS
    ...
    for i_cycle in range(params["MAXCYCLE"]):
        ...
        if seed_msa_clus is not None:
            ...
            if N_sample_more > 0:
                sample_more = sample_more[
                    torch.randperm(len(sample_more))[:N_sample_more]
                ]
        ...
        same_aa = torch.nn.functional.one_hot(
            msa_clust, num_classes=ChemData().NAATOKENS
        )
        ...
        msa_extra_onehot = torch.nn.functional.one_hot(
            msa_extra, num_classes=ChemData().NAATOKENS
        )
        count_clust = torch.logical_and(
            ~mask_pos, msa_clust != 20
        ).float()  # 20: index for gap, ignore both masked & gaps
        count_extra = torch.logical_and(~extra_mask, msa_extra != 20).float()
        agreement = torch.matmul(
            (count_extra[:, :, None] * msa_extra_onehot).view(N_extra, -1),
            (count_clust[:, :, None] * msa_clust_onehot).view(Nclust, -1).T,
        )
        ...
        count_profile = cluster_sum(
            count_extra[:, :, None], assignment, Nclust, L
        ).view(Nclust, L)
        ...
        ins_clust = (2.0 / np.pi) * torch.arctan(
            ins_clust.float() / 3.0
        )  # (from 0 to 1)
...
@pytest.mark.parametrize(
    "name,item,loader_params,chem_params,loader,loader_kwargs", test_data
)
def test_msa_featurize_regression(
    name, item, loader_params, chem_params, loader, loader_kwargs
):
    ChemData.reset()
    ...
    hash = item["HASH"]
    msa_fn = os.path.join(loader_params["PDB_DIR"], "a3m", hash[:3], f"{hash}.a3m.gz")
