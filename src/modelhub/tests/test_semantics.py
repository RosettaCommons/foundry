...
from modelhub.data.compose_dataset import compose_single_item_dataset
...
from modelhub.training.recycling import add_recycle_inputs, run_model_forward
from modelhub.util import is_atom
...
def test_fake_NC_position_noise(example, model):
    ...
    make_deterministic()
    rf_outputs_noise, rf_latents_noise = run_model_forward_noise(
        model, network_input, device=gpu
    )
    check_output(
        model_name,
        network_input,
...
def test_fake_sm_idx(example, model):
    ...
    make_deterministic()
    rf_outputs_noise, rf_latents_noise = run_model_forward(
        model, network_input_noise, device=gpu
    )
    check_output(
        model_name,
        network_input,
...
def test_mask_t(example, model):
    ...
    assert (
        torch.sum(~torch.eq(network_input["xyz_t"], network_input_noise["xyz_t"])) > 0
    ), "bug in test setup xyz_t should be different for base and noise case"
    make_deterministic()
    rf_outputs_noise, rf_latents_noise = run_model_forward(
        model, network_input_noise, device=gpu
...
def test_same_chain(example, model):
    ...
    rf_outputs, rf_latents = run_model_forward(model, network_input, device=gpu)
    network_input_noise = copy.deepcopy(network_input)
    network_input_noise["same_chain"] = torch.zeros_like(
        network_input_noise["same_chain"]
    )
    assert (
        torch.sum(
            ~torch.eq(network_input["same_chain"], network_input_noise["same_chain"])
        )
        > 0
    ...
    for name, output in rf_outputs.items():
        if torch.is_tensor(output):
            if name == "xyzs":
                ...
                mask = ChemData().allatom_mask.to(gpu)[network_input["seq_unmasked"]][
                    ..., :3
                ]
