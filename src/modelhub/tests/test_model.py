...
from modelhub.data.dataloader_adaptor import prepare_input
...
from modelhub.training.recycling import run_model_forward, run_model_forward_legacy
from modelhub.util import Ls_from_same_chain_2d, is_atom
...
test_conditions = setup_array(
    ["pdb", "na_compl", "rna", "sm_compl", "sm_compl_covale"],
    ["rf2aa", "rf_with_gradients", "untied_p2p", "rf2_deep_layerdropout"],
)
legacy_test_conditions = setup_array(
    ["pdb", "na_compl", "rna", "sm_compl", "sm_compl_covale"],
    ["legacy_train"],
    device=gpu,
)
...
def test_regression(example, model):
    ...
    make_deterministic()
    rf_outputs, rf_latents = run_model_forward(
        model, dataset_inputs, use_checkpoint=False, device=gpu
    )
    output_test = {"outputs": rf_outputs, "latents": rf_latents}
    model_pickle = model_pickle_path(dataset_name, model_name)
    match_values(
        output_test, model_pickle, model_name, dataset_name, output_test.keys()
    )
...
def test_regression_checkpoint(example, model):
    ...
    make_deterministic()
    rf_outputs, rf_latents = run_model_forward(
        model, dataset_inputs, use_checkpoint=True, device=gpu
    )
    output_test = {"outputs": rf_outputs, "latents": rf_latents}
    model_pickle = model_pickle_path(dataset_name, model_name)
    match_values(
...
def test_regression_legacy(example, model):
    dataset_name, dataset_inputs, model_name, model = setup_test(example, model)
    make_deterministic()
    output_i = run_model_forward_legacy(
        model, dataset_inputs, use_checkpoint=False, device=gpu
    )
    model_pickle = model_pickle_path(dataset_name, model_name)
    output_names = (
...
def grab_three_from_each_chain(network_input):
    ...
    first_in_chain = first_in_chain[
        first_in_chain < sum(Ls)
    ]  # remove cases where the chain is only one node

    node_from_each_chain = torch.zeros(
        sum(Ls), device=network_input["seq_unmasked"].device
    )
    node_from_each_chain[first_in_chain] = 1
    second_in_chain = first_in_chain + 1
    second_in_chain = second_in_chain[
        second_in_chain < sum(Ls)
    ]  # remove cases where the chain is only one node
    node_from_each_chain[second_in_chain] = 1
    third_in_chain = (
        second_in_chain + 1
    )  # se3 transofmer would like more than 1 edge during fwd pass
    third_in_chain = third_in_chain[
        third_in_chain < sum(Ls)
    ]  # remove cases where the chain is only one node
...
def downsample_network_inputs(network_input, chosen_nodes):
    ...
    network_input["t2d"] = network_input["t2d"][:, :, chosen_nodes][
        :, :, :, chosen_nodes
    ]
    network_input["xyz_t"] = network_input["xyz_t"][..., chosen_nodes, :]
    network_input["alpha_t"] = network_input["alpha_t"][..., chosen_nodes, :]
    network_input["mask_t"] = network_input["mask_t"][..., chosen_nodes, :][
        ..., chosen_nodes
    ]  # mask_t is t2d tensor
    network_input["same_chain"] = network_input["same_chain"][:, chosen_nodes][
        :, :, chosen_nodes
    ]
    network_input["bond_feats"] = network_input["bond_feats"][:, chosen_nodes][
        :, :, chosen_nodes
    ]
    network_input["dist_matrix"] = network_input["dist_matrix"][:, chosen_nodes][
        :, :, chosen_nodes
    ]
    network_input["xyz_prev"] = network_input["xyz_prev"][..., chosen_nodes, :, :]
    network_input["alpha_prev"] = network_input["alpha_prev"].to(
        network_input["seq_unmasked"].device
    )
    ...
    network_input["chirals"][..., :-1] = (
        network_input["chirals"][..., :-1] - num_deleted_residues
    )
            ...
            if output_names[idx] == "logits_c6d":
                for i in range(len(want)):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"{output_names[idx]} not same for model: {model_name} on dataset: {dataset_name}"
                        ) from e
            else:
                try:
            ...
            for output_name, output in output_regression[output_type].items():
                if torch.is_tensor(output):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"{output_name} does not match for model: {model_name} on dataset: {dataset_name}"
                        ) from e
