...
element_codes = [
    ...
    "Ubq",
    "Ubp",
    "Ubh",  # Atomic numbers 119-126
    "the_element",
    "of_surprise",  # Non-existant elements 127-128
]
...
def prepare_input_af3(
    ...
    **kwargs,
    ...
    element = [
        ChemData().aa2elt[seq[tok]][within_tok]
        for tok, within_tok in zip(tok_idx, within_tok_idx)
    ]
    f["ref_element"] = F.one_hot(
        torch.tensor(
            [
                element_code_to_num[e]
                if e in element_code_to_num
                else len(element_codes) - 1
                for e in element
            ]
        ),
        len(element_codes),
    ...
    f["ref_atom_name_chars"] = F.one_hot(
        inputs["ref_atom_name_chars"][0][is_real_atom].long(), num_classes=64
    )
    f["ref_space_uid"] = integer_tokenize(list(zip(f["asym_id"], f["residue_index"])))[
        tok_idx
    ]
    ...
    assert (
        template_seq < ChemData().NPROTAAS - 1
    ).all()  # only 20 AA + 1 UNK (No mask)

    f["template_restype"] = F.one_hot(af3num_from_num(template_seq), len(af3_num2aa))

    template_is_protein = torch.tensor(
        get_token_type(template_seq) == TokenType.PROTEIN
    )
    template_is_gly = template_seq == ChemData().aa2num["GLY"]
    template_atom_name = np.where(template_is_gly, " CA ", " CB ")
    template_protein_beta_idx = torch_vectorize(
        lambda token, atom_name: ChemData().aa2long[token].index(atom_name)
    )(template_seq[template_is_protein], template_atom_name[template_is_protein])
    template_beta_idx = torch.full((N_templ, N_token), 0)
    template_beta_idx[template_is_protein] = template_protein_beta_idx
    template_beta_exists = torch.gather(
        mask_t, dim=2, index=template_beta_idx[..., None]
    ).squeeze(-1)
    f["template_pseudo_beta_mask"] = (
        template_beta_exists * template_is_protein
    )  # .reshape?
    f["template_backbone_frame_mask"] = mask_t[:, :, torch.tensor([0, 1, 2])].all(
        dim=-1
    )
    ...
    template_pseudo_beta = torch.gather(
        xyz_t, dim=2, index=index_tensor_expanded
    ).squeeze(-2)  # .squeeze(-1)
    template_pseudo_beta_distogram = torch.cdist(
        template_pseudo_beta, template_pseudo_beta
    )
    template_pseudo_beta_distogram *= f["template_pseudo_beta_mask"].unsqueeze(-1) * f[
        "template_pseudo_beta_mask"
    ].unsqueeze(-2)
    f["template_distogram"] = F.one_hot(
        discretize_distance_matrix(
            template_pseudo_beta_distogram,
            num_bins=NUM_TEMPLATE_DISTOGRAM_BINS,
        ),
        num_classes=NUM_TEMPLATE_DISTOGRAM_BINS + 1,
    ...
    eps = 1e-4
    template_ca_disp_unit = template_ca_disp / (
        torch.linalg.norm(template_ca_disp, dim=-1, keepdim=True) + eps
    )
    ...
    is_protein_2d = (
        f["is_protein"].unsqueeze(-2) + f["is_protein"].unsqueeze(-1)
    ).bool()
    ...
    if t is None:
        ...
        warnings.warn(
            f"t is being set to {t} artificially; please make sure this is the desired behavior"
        )
...
def get_default_noise_schedule(t_init=0):
    ...
    return (
        sigma_data
        * (s_max ** (1 / p) + t_norm * (s_min ** (1 / p) - s_max ** (1 / p))) ** p
    )
