...
from rf2aa.kinematics import c6d_to_bins, xyz_to_c6d, xyz_to_t2d
from rf2aa.loss import *
from rf2aa.model.RoseTTAFoldModel import LegacyRoseTTAFoldModule as RoseTTAFoldModule
from rf2aa.symmetry import find_symm_subs, symm_subunit_matrix
from rf2aa.training.scheduler import (
    get_stepwise_decay_schedule_with_warmup,
)
...
def get_recycle_schedule(n_epochs, n_train, n_max, world_size):
    ...
    for i in range(n_epochs):
        recycle_schedule = [
            np.random.randint(1, n_max) for _ in range(n_train // world_size)
        ]
...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.recycle_schedule = get_recycle_schedule(
            self.n_epoch, self.n_train, self.maxcycle + 1, world_size
        )
    ...
    def calc_loss(
        ...
        for i in range(4):
            ...
            if i == 0:  # apply distogram loss to all residue pairs with valid BB atoms
                ...
                _, bb_frame_good = mask_unresolved_frames(
                    frames_BB, frame_mask_BB, mask_crds
                )  # (1, L, nframes)
        ...
        frame_atom_mask_2d_allatom = torch.einsum(
            "bfn,bra->bfnra", frame_mask_BB, mask_crds
        ).bool()  # B, L, nframes, L, natoms
        frame_atom_mask_2d = frame_atom_mask_2d_allatom[:, :, :, :, :3]
        frame_atom_mask_2d_intra_allatom = frame_atom_mask_2d_allatom * same_chain[
            :, :, None, :, None
        ].bool().expand(-1, -1, nframes, -1, NTOTAL)
        frame_atom_mask_2d_intra = frame_atom_mask_2d_intra_allatom[:, :, :, :, :3]
        different_chain = ~same_chain.bool()
        frame_atom_mask_2d_inter = frame_atom_mask_2d * different_chain[
            :, :, None, :, None
        ].expand(-1, -1, nframes, -1, 3)
        ...
        if "tf" in task[0] or res_mask.sum() == 0:
            ...
            tot_str, pae_loss, pde_loss = compute_general_FAPE(
                ...
                frame_atom_mask_2d=frame_atom_mask_2d_intra[:, res_mask[0]][
                    :, :, :, res_mask[0]
                ],
                dclamp=dclamp,
                logit_pae=logit_pae,
            ...
            dclamp_2d = torch.full_like(
                frame_atom_mask_2d_allatom, dclamp, dtype=torch.float32
            )
            if not unclamp:
                is_prot = is_protein(seq)  # (1,L)
                same_chain_clamp_mask = (
                    same_chain[:, :, None, :, None]
                    .bool()
                    .repeat(1, 1, nframes, 1, natoms)
                )
        ...
        w_bb_fape = torch.pow(
            torch.full((num_layers,), gamma, device=pred.device),
            torch.arange(num_layers, device=pred.device),
        )
        ...
        if (
            not negative
            and bool(torch.any(~sm_res_mask))
            and torch.any(frame_mask_BB[0, ~sm_res_mask])
        ):
            ...
            l_fape_prot_intra, _, _ = compute_general_FAPE(
                ...
                frame_atom_mask_2d=frame_atom_mask_2d_intra[:, ~sm_res_mask][
                    :, :, :, ~sm_res_mask
                ],
            )
            ...
            l_fape_prot_inter, _, _ = compute_general_FAPE(
                ...
                frames=frames_BB[:, ~sm_res_mask],
                frame_mask=frame_mask_BB[:, ~sm_res_mask],
                frame_atom_mask_2d=frame_atom_mask_2d_inter[:, ~sm_res_mask][
                    :, :, :, ~sm_res_mask
                ],
            )
        ...
        if not bool(torch.all(sm_res_mask)) and bool(torch.any(sm_res_mask)):
            ...
            if torch.any(mask_crds_protein[:, res_mask[0], :3]) and torch.any(
                frame_mask_BB_sm[:, res_mask[0]]
            ):
            ...
            if torch.any(mask_crds_sm[:, res_mask[0], :3]) and torch.any(
                frame_mask_BB_protein[:, res_mask[0]]
            ):
        ...
        true_tors, true_tors_alt, tors_mask, tors_planar = (
            self.xyz_converter.get_torsions(true, seq, mask_in=mask_crds)
        )
        ...
        if "tf" in task[0] or res_mask.sum() == 0:
            ...
            l_fape, _, _ = compute_general_FAPE(
                ...
                frame_atom_mask_2d=frame_atom_mask_2d_intra_allatom[:, res_mask[0]][
                    :, :, :, res_mask[0]
                ],
            )
        ...
        if torch.any(prot_mask_BB) and torch.any(mask_BB[0]):
            rmsd_prot_prot = calc_crd_rmsd(
                pred=pred_allatom[:, mask_BB[0], :, :3],
                true=nat_symm[None, mask_BB[0], :, :3],
                atom_mask=xs_mask_prot[:, mask_BB[0]],
                rmsd_mask=xs_mask_prot[:, mask_BB[0]],
            )
        else:
        ...
        if torch.any(not_prot_mask_BB) and torch.any(mask_BB[0]):
            rmsd_lig_lig = calc_crd_rmsd(
                pred=pred_allatom[:, mask_BB[0], :, :3],
                true=nat_symm[None, mask_BB[0], :, :3],
                atom_mask=xs_mask_lig[:, mask_BB[0]],
                rmsd_mask=xs_mask_lig[:, mask_BB[0]],
            )
        else:
            rmsd_lig_lig = torch.tensor([0], device=pred.device)
        if (
            torch.any(prot_mask_BB)
            and torch.any(not_prot_mask_BB)
            and torch.any(mask_BB[0])
        ):
        ...
        #     lj_lin=lj_lin
        # )
        clash_loss, num_violations = calc_l1_clash_loss(
            pred_allatom,
            seq[0],
            self.aamask,
            bond_feats,
        ...
        not_protein_mask_2d = torch.einsum(
            "l,r-> lr", not_prot_mask_BB, not_prot_mask_BB
        )
    ...
    def load_model(
        ...
        for param in msd_tgt:
            if param not in msd_src:
                ...
                if (
                    msd_src[param].shape[0] == 30
                    and msd_tgt[param].shape[0] == 32
                    and "compute_allatom_coords" not in param
                ):
                    if rank == 0:
                        print("Fixing", param)
    ...
    def run_model_training(self, world_size):
        ...
        if (
            ...
            if torch.cuda.device_count() == int(os.environ["SLURM_NTASKS"]):
                ...
                raise RuntimeError(
                    "Invalid distributed processing combination of nodes/tasks/gpus for SLURM."
                )
        else:
    ...
    def train_model(self, rank, world_size):
        ...
        train_sampler = DistributedWeightedSampler(
            ...
            fractions=OrderedDict(
                [(k, self.dataset_param["fraction_" + k]) for k in train_dict]
            ),
        ...
        valid_sets = dict(
            ...
            atomize_complex=Dataset(
                valid_ID_dict["atomize_complex"][
                    : self.dataset_param["n_valid_atomize_complex"]
                ],
                loader_atomize_complex,
                valid_dict["atomize_complex"],
                self.loader_param,
                homo,
                p_homo_cut=-1.0,
                n_res_atomize=9,
        ...
        valid_loaders = OrderedDict(
            [
                (
                    k,
                    data.DataLoader(
                        v, sampler=valid_samplers[k], **self.dataloader_kwargs
                    ),
                )
                for k, v in valid_sets.items()
            ]
            ...
            compl=(
                ...
                DatasetComplex(
                    valid_ID_dict["neg_compl"][
                        : self.dataset_param["n_valid_neg_compl"]
                    ],
                    loader_complex,
                    valid_dict["neg_compl"],
                    self.loader_param,
                    negative=True,
                ),
            ),
            na_compl=(
                ...
                DatasetNAComplex(
                    valid_ID_dict["neg_na_compl"][
                        : self.dataset_param["n_valid_neg_na_compl"]
                    ],
                    loader_na_complex,
                    valid_dict["neg_na_compl"],
                    self.loader_param,
                    negative=True,
                    native_NA_frac=0.0,
                ),
            ...
            [
                (
                    ...
                    (
                        ...
                        data.distributed.DistributedSampler(
                            w, num_replicas=world_size, rank=rank
                        ),
                    ),
                )
            ...
            [
                (
                    ...
                    (
                        data.DataLoader(
                            v,
                            sampler=valid_ppi_samplers[k][0],
                            **self.dataloader_kwargs,
                        ),
                        data.DataLoader(
                            w,
                            sampler=valid_ppi_samplers[k][1],
                            **self.dataloader_kwargs,
                        ),
                    ),
                )
        ...
        for epoch in range(loaded_epoch + 1, self.n_epoch):
            ...
            if (epoch % self.skip_valid == 0) or (epoch == loaded_epoch + 1):
                for dataset_name, valid_loader in valid_loaders.items():
                    valid_tot_, valid_loss_, valid_acc_, _ = self.valid_pdb_cycle(
                        ddp_model,
                        valid_loader,
                        rank,
                        gpu,
                        ...
                        epoch,
                        rng,
                        header=valid_headers[dataset_name],
                        verbose=self.eval,
                    )

                    if dataset_name == "sm_compl":
                        valid_tot, valid_loss, valid_acc = (
                            valid_tot_,
                            valid_loss_,
                            valid_acc_,
                        )
                ...
                for dataset_name, (
                    valid_pos_loader,
                    valid_neg_loader,
                ) in valid_ppi_loaders.items():
                    valid_tot_, valid_loss_, valid_acc_, _, _ = self.valid_ppi_cycle(
                        ddp_model,
                        valid_pos_loader,
                        valid_neg_loader,
                        rank,
                        ...
                        epoch,
                        rng,
                        header=valid_ppi_headers[dataset_name],
                        verbose=self.eval,
                    )
    ...
    def _prepare_input(self, inputs, gpu):
        ...
        xyz_prev = (
            INIT_CRDS.reshape(1, 1, NTOTAL, 3)
            .repeat(1, L, 1, 1)
            .to(gpu, non_blocking=True)
        )
        mask_prev = torch.zeros((1, L, NTOTAL), dtype=torch.bool).to(
        ...
        if symmgp != "C1":
            ...
            msa = torch.cat(
                [msa[:, :, :, :Lasu], *[msa[:, :, :, Lasu:]] * (Osub - 1)], dim=3
            )
            msa_masked = torch.cat(
                [
                    msa_masked[:, :, :, :Lasu],
                    *[msa_masked[:, :, :, Lasu:]] * (Osub - 1),
                ],
                dim=3,
            )
            msa_full = torch.cat(
                [msa_full[:, :, :, :Lasu], *[msa_full[:, :, :, Lasu:]] * (Osub - 1)],
                dim=3,
            )
            mask_msa = torch.cat(
                [mask_msa[:, :, :, :Lasu], *[mask_msa[:, :, :, Lasu:]] * (Osub - 1)],
                dim=3,
            )
            ...
            atom_frames = torch.cat(
                [atom_frames[:, :, :Lasu], *[atom_frames[:, :, Lasu:]] * (Osub - 1)],
                dim=2,
            )
            ...
            bond_feats_new = torch.zeros(
                (1, Osub * Lasu, Osub * Lasu), device=gpu
            ).long()
            dist_matrix_new = torch.zeros(
                (1, Osub * Lasu, Osub * Lasu), device=gpu
            ).long()
            for o_i in range(Osub):
                same_chain[
                    :, o_i * Lasu : (o_i + 1) * Lasu, o_i * Lasu : (o_i + 1) * Lasu
                ] = 1
        ...
        mask_t_2d = (
            mask_t_2d.float() * is_possible_t2d.float()[:, None]
        )  # (ignore inter-chain region between proteins)
        ...
        alpha, _, alpha_mask, _ = self.xyz_converter.get_torsions(
            xyz_t.reshape(-1, Lasu * Osub, NTOTAL, 3),
            seq_tmp,
            mask_in=mask_t.reshape(-1, Lasu * Osub, NTOTAL),
        )
        alpha = alpha.reshape(B, -1, Lasu * Osub, NTOTALDOFS, 2)
        alpha_mask = alpha_mask.reshape(B, -1, Lasu * Osub, NTOTALDOFS, 1)
        alpha_t = torch.cat((alpha, alpha_mask), dim=-1).reshape(
            B, -1, Lasu * Osub, 3 * NTOTALDOFS
        )
    ...
    def _get_loss_and_misc(
        ...
        if symmRs is not None:
            ...
            mapT2P = resolve_symmetry_predictions(
                pred_crds, true_crds, atom_mask, Lasu
            )  # (Nlayer, Ltrue)
            ...
            for li in logit_s:
                li = torch.gather(
                    li,
                    2,
                    mapT2P[-1][None, None, :, None].repeat(
                        1, li.shape[1], 1, li.shape[-1]
                    ),
                )
                li = torch.gather(
                    li,
            ...
            logit_aa_s = torch.gather(
                logit_aa_s,
                3,
                mapT2P[-1][None, None, None, :].repeat(
                    1, NAATOKENS, logit_aa_s.shape[-2], 1
                ),
            )
            logit_aa_s = logit_aa_s.view(1, NAATOKENS, -1)

            msa = torch.gather(
                msa, 2, mapT2P[-1][None, None, :].repeat(1, msa.shape[-2], 1)
            )
            mask_msa = torch.gather(
                mask_msa, 2, mapT2P[-1][None, None, :].repeat(1, mask_msa.shape[-2], 1)
            )

            logit_pae = torch.gather(
                logit_pae,
                2,
                mapT2P[-1][None, None, :, None].repeat(
                    1, logit_pae.shape[1], 1, logit_pae.shape[-1]
                ),
            )
            logit_pae = torch.gather(
                logit_pae,
                3,
                mapT2P[-1][None, None, None, :].repeat(
                    1, logit_pae.shape[1], logit_pae.shape[2], 1
                ),
            )

            logit_pde = torch.gather(
                logit_pde,
                2,
                mapT2P[-1][None, None, :, None].repeat(
                    1, logit_pde.shape[1], 1, logit_pde.shape[-1]
                ),
            )
            logit_pde = torch.gather(
                logit_pde,
                3,
                mapT2P[-1][None, None, None, :].repeat(
                    1, logit_pde.shape[1], logit_pde.shape[2], 1
                ),
            )

            pred_crds = torch.gather(
                pred_crds, 2, mapT2P[:, None, :, None, None].repeat(1, 1, 1, 3, 3)
            )
            pred_allatom = torch.gather(
                pred_allatom, 1, mapT2P[-1, None, :, None, None].repeat(1, 1, NTOTAL, 3)
            )
            alphas = torch.gather(
                alphas, 2, mapT2P[:, None, :, None, None].repeat(1, 1, 1, NTOTALDOFS, 2)
            )

            same_chain = torch.gather(
                same_chain,
                1,
                mapT2P[-1][None, :, None].repeat(1, 1, same_chain.shape[-1]),
            )
            same_chain = torch.gather(
                same_chain,
                2,
                mapT2P[-1][None, None, :].repeat(1, same_chain.shape[1], 1),
            )

            bond_feats = torch.gather(
                bond_feats,
                1,
                mapT2P[-1][None, :, None].repeat(1, 1, bond_feats.shape[-1]),
            )
            bond_feats = torch.gather(
                bond_feats,
                2,
                mapT2P[-1][None, None, :].repeat(1, bond_feats.shape[1], 1),
            )

            dist_matrix = torch.gather(
                dist_matrix,
                1,
                mapT2P[-1][None, :, None].repeat(1, 1, dist_matrix.shape[-1]),
            )
            dist_matrix = torch.gather(
                dist_matrix,
                2,
                mapT2P[-1][None, None, :].repeat(1, dist_matrix.shape[1], 1),
            )

            pred_lddts = torch.gather(
                pred_lddts,
                2,
                mapT2P[-1][None, None, :].repeat(1, pred_lddts.shape[-2], 1),
            )
            ...
            true_crds, atom_mask = resolve_equiv_natives(
                pred_crds[-1], true_crds, atom_mask
            )
    ...
    def train_cycle(
        ...
        for train_idx, inputs in enumerate(train_loader):
            regression = {"dataloader_inputs": inputs}
            (
                task,
                item,
                network_input,
            ...
            for i_cycle in range(N_cycle):
                with ExitStack() as stack:
                    ...
                    if i_cycle < N_cycle - 1:
                        continue
                    loss, loss_dict, acc_s, _, true_crds, pred_allatom, res_mask = (
                    ...
                    torch.save(regression, "test_pickles/model_io.pt")
            ...
            torch.save(
                ...
                "test_pickles/optimizer_regression.pt",
            )

            raise UnboundLocalError("stopping after 1 iteration")
            item_ = unbatch_item(
                item
            )  # remove nested lists to make more readable when printed
            save_pdbs = False
            if torch.isnan(loss):
            ...
            if task[0].startswith("sm_compl") or task[0].startswith("metal_compl"):
                if type(item_["LIGAND"][0]) is list:  # multires or covalent ligands
                    lig_str = "_".join(
                        [x[0] + x[1] + "-" + x[2] for x in item_["LIGAND"]]
                    )[:20]
                else:
                    lig_str = (
                        item_["LIGAND"][0]
            ...
            if save_pdbs:
                ...
                writepdb(
                    out_dir
                    + f"ep{epoch}_{task[0]}_{counter}.{rank}_{name}_xyz_prev.pdb",
                    torch.nan_to_num(xyz_prev_orig[res_mask][:, :23]),
                    network_input["seq_unmasked"][res_mask],
                    bond_feats=network_input["bond_feats"][:, res_mask[0]][
                        :, :, res_mask[0]
                    ],
                )
                writepdb(
                    out_dir
                    + f"ep{epoch}_{task[0]}_{counter}.{rank}_{name}_xyz_true.pdb",
                    torch.nan_to_num(true_crds[res_mask][:, :23]),
                    network_input["seq_unmasked"][res_mask],
                    bond_feats=network_input["bond_feats"][:, res_mask[0]][
                        :, :, res_mask[0]
                    ],
                ...
                writepdb(
                    out_dir
                    + f"ep{epoch}_{task[0]}_{counter}.{rank}_{name}_xyz_pred.pdb",
                    torch.nan_to_num(pred_allatom[res_mask][:, :23]),
                    network_input["seq_unmasked"][res_mask],
                    bond_feats=network_input["bond_feats"][:, res_mask[0]][
                        :, :, res_mask[0]
                    ],
            ...
            if counter == 1 and rank == 0:
                sys.stdout.write(
                    f"Header: [epoch/num_epochs] Batch: [examples_seen_in_epoch/examples_per_epoch] Time: time | Total_loss: total_loss | {' '.join(loss_dict.keys())} | precision recall F1 | max_mem \n"
                )
            ...
            if counter % self.ACCUM_STEP == 0:
                if rank == 0:
                    ...
                    if self.wandb_prefix is not None and rank == 0:
                        loss_dict.update(
                            {
                                "total_examples": epoch * len(train_loader)
                                + counter * world_size
                            }
                        )
    ...
    def valid_pdb_cycle(
            ...
            for inputs in valid_iter:
                ...
                for i_cycle in range(N_cycle):
                    with ExitStack() as stack:
                        ...
                        input_i = self._get_model_input(
                            network_input, output_i, i_cycle, gpu, return_raw=return_raw
                        )
                ...
                if save_pdbs:
                    ...
                    writepdb(
                        out_dir
                        + f"ep{epoch}_{task[0]}_{counter}.{rank}_{name}_true.pdb",
                        torch.nan_to_num(true_crds[res_mask][:, :23]),
                        seq_unmasked[res_mask],
                        bond_feats=network_input["bond_feats"][:, res_mask[0]][
                            :, :, res_mask[0]
                        ],
                        chain="A",
                        atom_mask=atom_mask[res_mask],
                    )

                    pred_sup = superimpose(
                        torch.nan_to_num(pred_allatom[:, res_mask[0], :23]),
                        torch.nan_to_num(true_crds[:, res_mask[0], :23]),
                        atom_mask[:, res_mask[0], :23],
                    )
                    writepdb(
                        out_dir
                        + f"ep{epoch}_{task[0]}_{counter}.{rank}_{name}_pred.pdb",
                        pred_sup,
                        seq_unmasked[res_mask],
                        bond_feats=network_input["bond_feats"][:, res_mask[0]][
                            :, :, res_mask[0]
                ...
                if self.eval:
                    record = OrderedDict(
                        name=name, Header=header, task=task[0], epoch=epoch
                    )
                    record.update({k: float(v) for k, v in loss_dict.items()})
                    logit_pae_ = (
                        logit_pae[..., res_mask[0]][..., res_mask[0], :]
                        if logit_pae is not None
                        else None
                    )
                    logit_pde_ = (
                        logit_pde[..., res_mask[0]][..., res_mask[0], :]
                        if logit_pde is not None
                        else None
                    )
                    pred_err = self.calc_pred_err(
                        pred_lddts, logit_pae_, logit_pde_, seq_unmasked[0, res_mask[0]]
                    )
        ...
        if rank == 0:
            ...
            sys.stdout.write(
                "%s: [%04d/%04d] Batch: [%05d/%05d] Time: %16.1f | total_loss: %8.4f | %s | %.4f %.4f %.4f\n"
                % (
                    header,
                    epoch,
                    self.n_epoch,
    ...
    def valid_ppi_cycle(
            ...
            for inputs in valid_pos_iter:
                ...
                for i_cycle in range(N_cycle):
                    with ExitStack() as stack:
                        ...
                        loss, loss_dict, acc_s, p_bind, _, _, _ = (
                            self._get_loss_and_misc(
                                output_i,
                                true_crds,
                                mask_crds,
                        ...
                        inter_s = torch.tensor(
                            [TP, 0.0, 0.0, FN], device=p_bind.device
                        ).float()
        ...
        if rank == 0:
            ...
            sys.stdout.write(
                ...
                % (
                    ...
                    epoch,
                    self.n_epoch,
                    len(valid_pos_loader) * world_size,
                    len(valid_pos_loader) * world_size,
                    train_time,
                    valid_tot,
                    " ".join(["%8.4f" % l for l in valid_loss]),
                ...
                for i_cycle in range(N_cycle):
                    with ExitStack() as stack:
                        ...
                        if p_bind > 0.5:
                            ...
                            FP, TN = 0.0, 1.0
                        inter_s = torch.tensor(
                            [0.0, FP, TN, 0.0], device=p_bind.device
                        ).float()
        ...
        if rank == 0:
            ...
            train_time = time.time() - start_time
            sys.stdout.write(
                "%s-discrim: [%04d/%04d] Batch: [%05d/%05d] Time: %16.1f | total_loss: %8.4f | %s | %.4f %.4f %.4f | %.4f %.4f %.4f\n"
                % (
                    header,
                    epoch,
                    self.n_epoch,
