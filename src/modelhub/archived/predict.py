...
from rf2aa.chemical import INIT_CRDS, NAATOKENS, NTOTAL, NTOTALDOFS
from rf2aa.data.data_loader import (
    MSAFeaturize,
    merge_a3m_hetero,
    merge_a3m_homo,
)
from rf2aa.data.parsers import parse_mixed_fasta, parse_multichain_fasta, read_templates
...
def get_args():
    ...
    parser.add_argument(
        "-checkpoint", default="./models/RF2_25c_last.pt", help="Path to model weights"
    )

    parser.add_argument(
        "-msa", help="Input sequence/MSA to predict structure from, in fasta/a3m format"
    )
    parser.add_argument("-pdb", help="PDB of sequence to predict structure from")
    parser.add_argument(
        "-multi", help="RFNA-formatted list of fasta/a3m files to predict structure"
    )
    parser.add_argument(
        "-oligo",
        help="homo-oligomeric state for each chain, when using multi",
        default=None,
        nargs="+",
    )
    ...
    parser.add_argument(
        "-mol2", help="mol2 of small molecule to predict structure from"
    )
    parser.add_argument(
        "-smiles", help="smiles string of small molecule to predict structure from"
    )  # fd unused
    parser.add_argument(
        "-db", default=DB, required=False, help="HHsearch database [%s]" % DB
    )
    ...
    parser.add_argument(
        "-outcsv", default="rfaa_scores.csv", help="output CSV for losses"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="output initial and final prediction in addition to best prediction",
    )
    parser.add_argument(
        "-dump_traj", action="store_true", default=False, help="output trajectory pdb"
    )
    parser.add_argument(
        "-dump_aux",
        action="store_true",
        default=False,
        help="output distograms/anglegrams and confidence estimates",
    )
    parser.add_argument(
        "-init_protein_tmpl",
        action="store_true",
        default=False,
        help="initialize protein template structure to ground truth",
    )
    parser.add_argument(
        "-init_ligand_tmpl",
        action="store_true",
        default=False,
        help="initialize ligand template structure to ground truth",
    )
    parser.add_argument(
        "-init_protein_xyz",
        action="store_true",
        default=False,
        help="initialize protein coordinates to ground truth",
    )
    parser.add_argument(
        "-init_ligand_xyz",
        action="store_true",
        default=False,
        help="initialize ligand coordinates to ground truth",
    )
    parser.add_argument(
        "-num_interp",
        type=int,
        default=5,
        help="number of interpolation frames for trajectory output",
    )
    parser.add_argument(
        "-n_pred", type=int, default=1, help="number of repeat predictions"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="residues to truncate at N-term on MSA to match PDB",
    )
    parser.add_argument(
        "-trunc_C",
        type=int,
        default=0,
        help="residues to truncate at C-term on MSA to match PDB",
    )
    parser.add_argument(
        "-use_chiral_l1",
        type=bool,
    ...
    parser.add_argument(
        "-atomize_residues", default=None, required=False, help="Residues to atomize"
    )
    parser.add_argument(
        "-disulfidize_residues",
        default=None,
        required=False,
        help="Residues to atomize",
    )

    # fd: symmetry
    parser.add_argument(
        "-symm", required=False, default="C1", help="Model with symmetry"
    )
    parser.add_argument(
        "-symm_fit",
        required=False,
...
def calc_pred_err(pred_lddts, logit_pae, logit_pde, seq):
    ...
    inter_mask_2d = (
        sm_mask[None, :] * (~sm_mask[:, None]) + (~sm_mask[None, :]) * sm_mask[:, None]
    )
    # assumes B=1
    err_dict = dict(
...
def atomize_protein(resids, seq, dslf=None):
    ...
    lig_seq = torch.tensor(
        [
            aa2num[residues_atom_types[r][a]]
            if residues_atom_types[r][a] in aa2num
            else aa2num["ATM"]
            for r, a in ra
        ]
    )
...
def load_prot_na_data(fasta_fn, oligo=None):
    ...
    for i, seq_i in enumerate(fasta_fn):
        ...
        if ftype.upper() == "PR":
            ...
            msa_i, ins_i, L = parse_multichain_fasta(
                fseq_i,
                rna_alphabet=(ftype.upper() == "R"),
                dna_alphabet=(ftype.upper() == "D"),
            )
    ...
    for i in range(1, len(f_Ls)):
        msa_orig = merge_a3m_hetero(
            msa_orig, {"msa": msas[i], "ins": inss[i]}, [sum(f_Ls[:i]), f_Ls[i]]
        )
...
class Predictor:
    def __init__(self, args, device="cuda:0"):
        ...
        self.ffdb = FFindexDB(
            read_index(args.db + "_pdb.ffindex"), read_data(args.db + "_pdb.ffdata")
        )
    ...
    def predict(
        ...
        if pdb_fn is not None:
            msa_prot, ins_prot, Ls_prot, xyz, mask_prot, _, dslf = (
                parsers.read_multichain_pdb(pdb_fn, tmpl_chain=tmpl_chain)
            )
            ...
            if len(stream) > 0:
                mol, msa_sm, ins_sm, xyz_sm, mask_sm = parsers.parse_mol(
                    "".join(stream), filetype="pdb", string=True
                )
        ...
        if fasta_fn is not None:
            msa_prot, ins_prot, chain_idx, idx_pdb, Ls = load_prot_na_data(
                fasta_fn.split(","), oligo
            )
            protein_L = msa_prot.shape[-1]
            prot_na = True
        else:
        ...
        if smiles is not None:
            a3m_prot = {"msa": msa_prot, "ins": ins_prot}
            mol, msa_sm, ins_sm, xyz_sm, mask_sm = parsers.parse_mol(
                smiles, filetype="smiles", string=True, generate_conformer=True
            )
            a3m_sm = {"msa": msa_sm.unsqueeze(0), "ins": ins_sm.unsqueeze(0)}
            G = util.get_nxgraph(mol)
            atom_frames = util.get_atom_frames(msa_sm, G)
        ...
        if len(atomize_res) > 0 or len(disulfidize_res) > 0:
            ...
            lig_seq, lig_ins, atom_frames, bond_feats_sm, chirals, prot_atom_conns = (
                atomize_protein(atomize_res, msa_prot[0], dslf=disulfidize_res)
            )
            ...
            bond_feats[: Ls[0], : Ls[0]] = util.get_protein_bond_feats_from_idx(
                LprotOrig, idx_prot
            )
        ...
        xyz_t_cloud[:, NAmask] = (
            ...
            + torch.rand(1, NAmask.sum(), 1, 3) * random_noise
            - random_noise / 2
            + SYMM_OFFSET_SCALE
            * symmoffset
            * NAmask.sum() ** (1 / 2)  # note: offset based on symmgroup
        )

        Pmask = ~NAmask
        xyz_t_cloud[:, Pmask] = (
            INIT_CRDS.reshape(1, 1, NTOTAL, 3).repeat(1, Pmask.sum(), 1, 1)
            + torch.rand(1, Pmask.sum(), 1, 3) * random_noise
            - random_noise / 2
            + SYMM_OFFSET_SCALE
            * symmoffset
            * Pmask.sum() ** (1 / 2)  # note: offset based on symmgroup
        )
        ...
        if init_protein_tmpl or init_ligand_tmpl:
            ...
            f1d_t = torch.cat(
                (
                    torch.nn.functional.one_hot(
                        torch.full((2, sum(Ls)), 20).long(), num_classes=NAATOKENS - 1
                    ).float(),  # all gaps (no mask token)
                    torch.zeros((2, sum(Ls), 1)).float(),
                ),
                -1,
            )  # (2, L_protein + L_sm, NAATOKENS)
            ...
            if init_protein_tmpl:  # input true protein xyz as template 0
                ...
                f1d_t[0, : Ls[0]] = torch.cat(
                    (
                        torch.nn.functional.one_hot(
                            msa[0, : Ls[0]], num_classes=NAATOKENS - 1
                        ).float(),
            ...
            if init_ligand_tmpl:  # input true s.m. xyz as template 1
                ...
                f1d_t[1, Ls[0] :] = torch.cat(
                    (
                        torch.nn.functional.one_hot(
                            msa[0, Ls[0] :] - 1, num_classes=NAATOKENS - 1
                        ).float(),
                        templ_conf * torch.ones((Ls[1], 1)).float(),
                    ),
                    -1,
                )  # (1, L_sm, NAATOKENS)
                mask_t[1, Ls[0] :, 1] = mask_sm[
                    0
                ]  # all symmetry variants have same mask
        elif hhr_fn is not None:
            # templates from file
            xyz_t_prot, mask_t_prot, t1d_prot = read_templates(
                qlen, self.ffdb, hhr_fn, atab_fn, n_templ=n_templ
            )
            ...
            f1d_t = torch.cat(
                (
                    torch.nn.functional.one_hot(
                        ...
                        num_classes=NAATOKENS - 1,
                    ).float(),  # all gaps (no mask token)
                    torch.zeros((n_templ, sum(Ls), 1)).float(),
                ),
                -1,
            )  # (n_templ, L_protein + L_sm, NAATOKENS)
            ...
            f1d_t = torch.nn.functional.one_hot(
                torch.full((1, sum(Ls)), 20).long(), num_classes=NAATOKENS - 1
            ).float()  # all gaps
        ...
        if init_protein_xyz or init_ligand_xyz:
            ...
            xyz_prev[: sum(Ls)] = torch.where(
                mask_prev[:, :, None], xyz_prev[: sum(Ls)], xyz_t_cloud
            ).contiguous()
        ...
        seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
            ...
            ins,
            p_mask=0.0,
            params={"MAXLAT": MAXLAT, "MAXSEQ": MAXSEQ, "MAXCYCLE": n_cycle},
            tocpu=True,
        )
        ...
        alpha, _, alpha_mask, _ = self.xyz_converter.get_torsions(
            xyz_t.reshape(-1, sum(Ls), NTOTAL, 3), seq_tmp
        )
        ...
        alpha_t = (
            torch.cat((alpha, alpha_mask), dim=-1)
            .reshape(1, -1, Osub * sum(Ls), 3 * NTOTALDOFS)
            .to(self.device)
        )
        ...
        with torch.no_grad():
            ...
            for i_cycle in tqdm(range(n_cycle)):
                seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
                    ...
                    ins,
                    p_mask=0.0,
                    params={"MAXLAT": MAXLAT, "MAXSEQ": MAXSEQ, "MAXCYCLE": 1},
                    tocpu=True,
                )
                ...
                if prot_na:
                    ...
                    print(
                        "RECYCLE %2d %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f"
                        % (
                            i_cycle,
                            pae_pp,
                            pae_pd,
                    ...
                    print(
                        f"RECYCLE {i_cycle}\tcurrent lddt: {pred_lddt.mean():.3f}\t"
                        f"best lddt: {best_lddt.mean():.3f}"
                    )

                err_dict = calc_pred_err(
                    pred_lddt_binned, logit_pae, logit_pde, seq_network[0]
                )
                loss_dict = {}
        ...
        for recycle in range(n_cycle):
            ...
            bond_featsfull = torch.zeros(
                (B, O * Lasu, O * Lasu), device=best_lddt.device
            )
            bond_featsfull[:, :Lasu, :Lasu] = bond_feats[:, :Lasu, :Lasu]
            Lsfull = Ls * O
            for i in range(1, O):
                best_xyzfull[:, (i * Lasu) : ((i + 1) * Lasu)] = torch.einsum(
                    "ij,braj->brai", symmRs[i], best_xyz[:, :Lasu].float()
                )
                bond_featsfull[
                    :, (i * Lasu) : ((i + 1) * Lasu), (i * Lasu) : ((i + 1) * Lasu)
                ] = bond_feats[:, :Lasu, :Lasu]
                seq_full[:, 0, (i * Lasu) : ((i + 1) * Lasu)] = seq_network[None][
                    :, 0, :Lasu
                ]

            util.writepdb(
                out_prefix + f"_rec{recycle}.pdb",
                best_xyzfull[0],
                seq_full[0, -1],
                bfacts=100 * best_lddtfull[0].float(),
        ...
        if args.dump_extra_pdbs:
            util.writepdb(
                out_prefix + "_last.pdb",
                xyz_prev[0],
                seq[0, -1],
                bfacts=best_lddtfull[0].float(),
                bond_feats=bond_featsfull,
            )
            util.writepdb(
                out_prefix + "_init.pdb",
                xyz_prev_orig[0],
                seq[0, -1],
                bond_feats=bond_featsfull,
            )
        ...
        if args.dump_aux:
            prob_s = [
                prob.permute(1, 2, 0).detach().cpu().numpy().astype(np.float16)
                for prob in prob_s
            ]
                ...
                for i_atom in range(3):
                    for i_coord in range(3):
                        interp = Akima1DInterpolator(
                            t,
                            all_pred[:, 0, i_res, i_atom, i_coord]
                            .detach()
                            .cpu()
                            .numpy(),
                        )
                        Y[:, i_res, i_atom, i_coord] = interp(
                            np.arange(n_frames) / args.num_interp
                        )
            Y = torch.from_numpy(Y).float()

            # 1st frame is final pred so pymol renders bonds correctly
            util.writepdb(
                out_prefix + "_traj.pdb",
                Y[-1],
                seq[0, -1],
                modelnum=0,
                bond_feats=bond_feats,
...
if __name__ == "__main__":
    ...
    if args.out is None:
        if args.msa is not None:
            ...
            in_name = "_".join(
                [os.path.basename(x.split(":")[-1]) for x in args.multi.split(",")]
            )
        ...
        for fn in filenames:
            ...
            for n in range(args.n_pred):
                ...
                loss_dict = pred.predict(
                    outdir + name + f"_pred_{n}", pdb_fn=fn, n_cycle=args.n_cycle
                )
                loss_dict["name"] = name + f"_{n}"
                print(
                    f"rmsd_c1_c1: {loss_dict['rmsd_c1_c1']:.3f}\t"
