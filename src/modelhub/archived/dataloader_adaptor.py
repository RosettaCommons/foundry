...
def prepare_input(inputs, xyz_converter, gpu):
    ...
    mask_prev = torch.zeros((1, L, ChemData().NTOTAL), dtype=torch.bool).to(
    ...
    alpha, _, alpha_mask, _ = xyz_converter.get_torsions(
        xyz_t.reshape(-1, Lasu * Osub, ChemData().NTOTAL, 3),
        seq_tmp,
        mask_in=mask_t.reshape(-1, Lasu * Osub, ChemData().NTOTAL),
    )
    ...
    alpha_t = torch.cat((alpha, alpha_mask), dim=-1).reshape(
        B, -1, Lasu * Osub, 3 * ChemData().NTOTALDOFS
    )
    alpha_prev = torch.zeros((B, Lasu * Osub, ChemData().NTOTALDOFS, 2)).to(
        gpu, non_blocking=True
    )
...
def get_loss_calc_items(inputs, device="cpu"):
    ...
    return (
        seq.to(device),
        same_chain.to(device),
        idx_pdb.to(device),
        bond_feats.to(device),
        dist_matrix.to(device),
        atom_frames.to(device),
        true_crds.to(device),
        mask_crds.to(device),
    )
...
def prepare_input_fm(inputs, interpolant, xyz_converter, device="cpu"):
    ...
    batch = data_transforms.convert_dataloader_inputs_to_rigids(
        inputs, interpolant._device
    )
...
def prepare_input_fm_allatom(inputs, interpolant, xyz_converter, device="cpu"):
    ...
    trans_t = interpolant._corrupt_trans(
        trans_1[None], t, atom_mask[None], diffuse_mask[None]
    )
...
def construct_template_feats(
    xyz_t, mask_t, t1d, seq, atom_frames, xyz_converter, use_atom_frames=False
):
    ...
    if use_atom_frames:
        raise NotImplementedError(
            "use_atom_frames is not implemented for get template features"
        )
        xyz_t_frame = xyz_t_to_frame_xyz(xyz_t, msa[:, 0, 0], atom_frames)
    else:
        xyz_t_frame = xyz_t
