...
class LossTestCase(unittest.TestCase):
    ...
    def test_compute_general_FAPE(self):
        with self.subTest("test that FAPE loss is correctly calculated for proteins"):
            for (
                ...
                l_fape = compute_general_FAPE(
                    true_crds, true_crds, atom_mask, frames, frame_mask
                )
                ...
                for i in range(5):
                    ...
                    l_fape = compute_general_FAPE(
                        perturbed_crds, true_crds, atom_mask, frames, frame_mask
                    )
                    fapes.append(l_fape)
                for i in range(1, 5):
        ...
        with self.subTest(
            "test that FAPE loss is correctly calculated for protein/NA complexes"
        ):
            for (
                seq,
                msa,
                ...
                break
        with self.subTest(
            "test that FAPE loss is correctly calculated for protein/SM complexes"
        ):
            for (
                seq,
                msa,
                ...
                atom_frames,
                bond_feats,
            ) in self.valid_sm_compl_loader:
                # first assert that same structure gives you 0 loss
                true_crds, atom_mask = resolve_equiv_natives(
                    true_crds[0, 0].unsqueeze(0), true_crds, atom_mask
                )
                frames, frame_mask = get_frames(
                    true_crds, atom_mask, msa[:, 0, 0], frame_indices, atom_frames
                )
                ...
                break
        with self.subTest(
            "test that protein backbone FAPE loss can be calculated with compute_general_FAPE"
        ):
            for (
                seq,
                msa,
                ...
                res_mask = ~(
                    (atom_mask[:, :, :3].sum(dim=-1) < 3.0) * ~(is_atom(msa[:, 0, 0]))
                )
                ...
                for i in range(5):
                    ...
                    tot_str, str_loss = calc_str_loss(
                        perturbed_crds.unsqueeze(0),
                        true_crds,
                        mask_2d,
                        same_chain,
                        negative=False,
        ...
        for (
            ...
            frame_mask *= torch.all(
                torch.gather(
                    ...
                    1,
                    frames_reindex.reshape(1, L * NFRAMES * 3),
                ).reshape(1, L, -1, 3),
                axis=-1,
            )
    ...
    def test_resolve_equiv_natives(self):
        ...
        for (
            ...
            true_crds, atom_mask = resolve_equiv_natives(
                torch.randn(true_crds[0, 0].unsqueeze(0).shape), true_crds, atom_mask
            )
            print(true_crds)
            break
...
class DataLoaderTestCase(unittest.TestCase):
    ...
    def test_vaporize_protein(self):
        for (
            ...
            true_alt.scatter_(
                2, long2alt[msa[:, 0, 0], :, None].repeat(1, 1, 1, 3), true_crds
            )
            coords_stack = torch.stack(
                (true_crds[:, i - 2 : i + 2], true_alt[:, i - 2 : i + 2]), dim=0
            )
            swaps = (
                (coords_stack[0] == coords_stack[1]).all(dim=2).all(dim=2).squeeze()
            ...
            if swaps.numel() != 0:
                ...
                stack = torch.stack(
                    (combs, swaps.unsqueeze(0).repeat(swaps.numel() + 1, 1)), dim=2
                )
                coords_stack = coords_stack.repeat(
                    swaps.numel() + 1, 1, 1, 1, 1
                ).squeeze(1)
            ...
            msa_masked = torch.cat(
                (msa_masked[:, :, :, : i - 2], msa_masked[:, :, :, i + 2 :]), dim=3
            )
            msa_full = torch.cat(
                (msa_full[:, :, :, : i - 2], msa_full[:, :, :, i + 2 :]), dim=3
            )
            mask_msa = torch.cat(
                (mask_msa[:, :, :, : i - 2], mask_msa[:, :, :, i + 2 :]), dim=3
            )
            ...
            same_chain = torch.cat(
                (same_chain[:, : i - 2], same_chain[:, i + 2 :]), dim=1
            )
            same_chain = torch.cat(
                (same_chain[:, :, : i - 2], same_chain[:, :, i + 2 :]), dim=2
            )

            bond_feats = torch.cat(
                (bond_feats[:, : i - 2], bond_feats[:, i + 2 :]), dim=1
            )
            bond_feats = torch.cat(
                (bond_feats[:, :, : i - 2], bond_feats[:, :, i + 2 :]), dim=2
            )
    ...
    def test_atomized_pdb_loader(self):
        for (
            ...
            atom_frames,
            bond_feats,
        ) in self.valid_atomize_pdb_loader:
            print(seq.shape)
            print(msa.shape)
