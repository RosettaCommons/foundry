...
import util
from data_loader import MSABlockDeletion, MSAFeaturize
from ffindex import *
...
class Predictor:
    ...
    def run_prediction(
        ...
        with torch.no_grad():
            ...
            for i_cycle in range(MAX_CYCLE):
                ...
                self.write_pdb(
                    ...
                    init_allatom[-1],
                    Bfacts=pred_lddt[0],
                    prefix="%s_cycle_%02d" % (out_prefix, i_cycle),
                )
        ...
        self.write_pdb(
            seq[0, -1], best_xyz, Bfacts=best_lddt[0], prefix="%s_init" % (out_prefix)
        )
        prob_s = [
...
def get_args():
    ...
    parser.add_argument(
        "-i", default=1, type=int, required=False, help="Parallelize i of j [1]"
    )
    parser.add_argument(
        "-j", default=1, type=int, required=False, help="Parallelize i of j [1]"
    )
