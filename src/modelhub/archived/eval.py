...
from rf2aa.chemical import NAATOKENS, NTOTAL, NTOTALDOFS
from rf2aa.data.data_loader import MSAFeaturize, merge_a3m_homo
from rf2aa.data.parsers import parse_fasta, read_template_pdb
from rf2aa.ffindex import *
from rf2aa.kinematics import get_init_xyz, xyz_to_t2d
    ...
    def predict(
        ...
        if tmpl_fn and os.path.exists(tmpl_fn):
            ...
            t1d = torch.nn.functional.one_hot(
                torch.full((1, L), 20).long(), num_classes=NAATOKENS - 1
            ).float()  # all gaps
    ...
    def run_prediction(
        ...
        with torch.no_grad():
            ...
            seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
                ...
                ins,
                p_mask=0.1,
                params={"MAXLAT": 128, "MAXSEQ": 1024, "MAXCYCLE": MAX_CYCLE},
                tocpu=True,
            )
            ...
            for i_cycle in range(MAX_CYCLE):
                ...
                _, all_crds = self.compute_allatom_coords(
                    seq[:, i_cycle], init_crds[-1], alpha_prev
                )
