...
def add_ground_truth_reference_conformer( 
    ...
    ref_pos_is_ground_truth: Bool[np.ndarray, "L"]
    ...
    """
    assert isinstance(ref_pos, np.ndarray), "ref_pos must be a np array; ensure that this Transform is called before ConvertToTorch"
...
def noise_residues_with_ground_truth_conformer(atom_array: AtomArray, p: float = 0.5, sigma: float = 0.5) -> AtomArray:
    ...
    ref_pos_will_be_ground_truth = atom_array.ground_truth_conformer_policy == GroundTruthConformerPolicy.REPLACE
    ...
    noise = np.random.normal(scale=sigma, size=(np.sum(ref_pos_will_be_ground_truth), 3))
...
class NoiseResiduesWithGroundTruthConformer(Transform):
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        data["atom_array"]= noise_residues_with_ground_truth_conformer(data["atom_array"], p=self.p, sigma=self.sigma)
        return data
